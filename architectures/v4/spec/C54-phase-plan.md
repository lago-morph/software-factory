...
- **OQ-3 (→ review-log, G31; aligns with XC-8; DEFERRED — needs orchestrator decision).** Should C43 (isolation/lethal-trifecta boundary) be **pulled forward** from its faithful P3c home, so the factory has a blast-radius bound before unattended-scale operation (P2) and before self-modification (P3b)? The faithful manifest keeps C43 at P3c + a Caution; the engineering recommendation is to pull it forward. **Refinement (adversary, sweep-1): C43 is two separable mechanisms and they should not share one phase.** Per inventory, C43 `Depends on: C42, C44`; only its **twin-isolation** half is structurally blocked on C44 (twins, genuinely P3c) — that half legitimately stays at P3c. Its **deterministic-boundary-typing / blast-radius** half (the Bash/net/fs capability bound; D-13's "Bash/net/fs typing") depends only on C42 (Batch 2) + the P4 reconciler/tool-node primitives (available P0), **not** on twins, and so is *not* structurally blocked. Recommendation: **pull the boundary-typing half forward to a P2 entry precondition; keep twin-isolation at P3c** — or, per XC-8, explicitly **"accept detection-only Phase 0"**. This is the single most consequential sequencing decision in the plan and is the integrator's call; the faithful manifest is deliberately left un-re-sequenced. (XC-8: "Real fix = sequence C43 earlier or accept detection-only Phase 0.")
