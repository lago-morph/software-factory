...
| **G39** counts ≠ 61 | §10 headline 24+20+11+4 = 59; the **raw rows pre-dedup read 30 "Addressed" + 18 "Partial"** (G39 in `ambiguities-and-gaps.md` — the §7 "overlap" row + the F32/F35/F47 double-counts inflate the row tally over the headline) | After adding F15 and de-duplicating F32/F35/F47, **the per-status counts are recomputed to sum to 61** and the arithmetic is **shown** (derivable from rows). The re-tally must reconcile *both* the 59-headline and the 30/18 pre-dedup row reality. The headline percentages are recomputed against 61, not an unreconciled base. |
...
| **F54 / G35 objective-drift audit** | **C57 (home) — UNBUILT, Batch 5** | the multi-cycle goal-comparison audit does not exist; C57 is its declared register home (C52:OQ6/C56:OQ-3/C39:OQ3) but registers it unbuilt. **Wrap-up decision (D-21, ADOPTED 2026-05-31):** F54 stays a **registered residual**; mitigated NOW by a **cheap periodic human checkpoint** ("do the objectives still match intent?") tied to each batched human-review point on the C56 autonomy ladder; a **real drift detector is REQUIRED before L5 lights-out** and is a hard precondition on L5 promotion (C56). See [the decision ledger](../_meta/review-log.md#wrap-up-operator-decisions-2026-05-31--d-20d-25). |
...
| **Flag-lib (Unleash) license pin** | C57 license-hygiene register (+ Sweep-2 version-pin); mirrors C27 LangFuse SPDX-drift | **Wrap-up decision (D-25, ADOPTED 2026-05-31):** pin a **known-open (Apache-2.0) version of Unleash, or an unambiguously-open alternative**; the license-census home for this verdict is this register + the Sweep-2 version-pin step. See [the decision ledger](../_meta/review-log.md#wrap-up-operator-decisions-2026-05-31--d-20d-25). |
...
- **OQ-C57-3** (→ review-log; **top open question**): **Does the F54 / objective-drift audit get a real
  ...
  **Wrap-up decision (D-21, ADOPTED 2026-05-31):** F54 stays a **registered residual** here; mitigated now by a **cheap periodic human checkpoint** tied to each C56 batched-review point; a **real drift detector is REQUIRED before L5 lights-out** (hard precondition on L5 promotion, C56). The "build it now" alternative is not taken — this is NOT a Sweep-1 build. See [the decision ledger](../_meta/review-log.md#wrap-up-operator-decisions-2026-05-31--d-20d-25).
...
The enforcement watcher is a sanctioned-but-undesigned residual in this register: its NEED is gated on the D-23 spike and its DESIGN is deferred until the spike confirms the substrate does not prevent.
