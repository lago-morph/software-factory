...
- `scripts/mhtml_extract.py` was written for this pass and is committed alongside this index. It supports `info` (JSON metadata) / `text` (plain body) / `save-image` (extract Nth embedded image) / `to-txt` (write a clean .txt with TITLE/URL header). Future drains can reuse it.
