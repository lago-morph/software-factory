...
> [FAITHFUL-FILL] **The nine field names are inferred, not enumerated in v4.** F-MODE:91 asserts "9-field structured intake" and names the source (GF-C) but lists **no field names anywhere in the four source docs**, and **GF-C is never defined** (the only other GF-C mention, F-MODE:19, is an unrelated signed-scenarios pattern). The *minimal consistent* field set is the one that operationalizes the spec attributes one-shot Part 2 names as causally affecting code-gen success — **completeness, ambiguity, specificity, requirement classes** (lines 86–122) — plus the **definition-of-done / acceptance-criteria** practice the Part 1 corpus actually shows (Kilroy `DoD.md`). The set below is that minimal mapping; the *count* (9) is faithful to F-MODE:91, the *names* are the FAITHFUL-FILL and are the integrator's to confirm against the real GF-C exemplar at sweep 2 (→ OQ-1).
...
| 9 | **Exemplar / transfusion reference** | The concrete external example to behave like (the gene-transfusion anchor) + its provenance. | C51 gene-transfusion discipline; A93/A107 lineage via inventory map. |
