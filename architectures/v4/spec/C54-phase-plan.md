...
| **P3c** | P7 digital twins per service (C44/C45) | LocalStack (README:468) | "Repeat per dependency." |
...
> **Faithful pick: Reading A.** For C54 specifically the ambiguity is *contained*: C54's own scheme is **Phases**, and the only "Layer" token that appears in Part 6 is **"Layer 2 (scenarios + judge)"** in the P2 heading (README:417) — which Reading A pins unambiguously to the scenarios/judge tier. So **within C54, "Layer 2" = the P2 scenarios+judge deliverable**, and C54 carries the cross-walk below so no reader conflates the three schemes. The architecture-wide reconciliation of the two layer vocabularies is bigger than C54 (it touches every doc using "layer") and is raised as OQ-1 for the integrator / C57.
...
- **v4's implicit placement.** Twins (C44/C45, P7) land at **P3c**. Deterministic boundary typing / the lethal-trifecta boundary (C43, P4-adjacent) is *named as a P4 capability* ("deterministic-first") but **no phase is ever stated for C43's actual build**; F12/F44/F56 are marked "Addressed" in F-MODE-COVERAGE on the strength of mechanisms that ship at P3c or are unbuilt (G31).
...
> - **Reading B (engineering-corrected).** Pull C43 *earlier* — to **P1 or P2** — so the factory has a blast-radius bound before it runs unattended at scale and before it self-modifies (P3b). This is what XC-8 and the C43/D-13 owners argue ("Real fix = sequence C43 earlier or accept detection-only Phase 0").
> **Faithful pick: Reading A** (place C43 at P3c per v4's literal twin-sequencing) **but with a mandatory, visible Caution**, because the canonical track elaborates v4 *as written* and must not silently re-sequence a security control. The *recommendation* to pull C43 forward is the right engineering call and is recorded as **OQ-3 → review-log** (and aligns with XC-8). C54's contribution to G31 is therefore: (1) name C43's faithful phase (P3c), (2) make the P0–P3b exposure window explicit instead of "Addressed", (3) flag the pull-forward as the open decision. (G31 partially resolved here — sequenced + exposure surfaced; the *fix* is C43's build + the integrator's call on pulling it forward.)
...
- **OQ-3 (→ review-log, G31; aligns with XC-8).** Should C43 (isolation/lethal-trifecta boundary) be **pulled forward** from its faithful P3c home to P1/P2, so the factory has a blast-radius bound before unattended-scale operation (P2) and before self-modification (P3b)? The faithful manifest keeps C43 at P3c + a Caution; the engineering recommendation is to pull it forward. This is the single most consequential sequencing decision in the plan and is the integrator's call. (XC-8: "Real fix = sequence C43 earlier or accept detection-only Phase 0.")
