...
| T4 | **Cost/usage signal read + cost-vector assembly (I2/INV-3)** — read recorded per-run usage (tokens, latency) from the telemetry path (C24/CXDB); derive dollars = tokens × reference price (C29); assemble + attribute the {tokens,$,time} vector. **Read + derive only** — no metering. | M | T1, T3, C24 read API |
