...
1. **A five-level maturity ladder** (Levels 0–5), borrowed from Dan Shapiro, that grounds "dark factory" as Level 5 of a continuum rather than a binary state. *"Most teams will not reach Level 5. Most teams would benefit enormously from reaching Level 4."*
   - **Note on attribution (added 2026-05-13 after canonical Shapiro post drained — see [`01-shapiro-five-levels`](followup/01-shapiro-five-levels.md)):** El Kaim's restatement of the five-level framework *conflates Shapiro's canonical "Five Levels" post with his companion post "You don't write the code"* and adds his own compressions. Specifically: Shapiro's actual canonical L5 text is just *"It's a black box that turns specs into software."* The "nobody writes / nobody reads / lights are off / working software accumulates" expansion quoted later in this report (§4 Level 5 + §"Notable quotes" item 5) is El Kaim's framing, not Shapiro verbatim — it derives partly from the companion post's "you don't write / you don't read" framing and partly from El Kaim's own essay structure. Likewise, El Kaim's L4 framing ("the spec is the most valuable thing you produce") is his editorial compression; Shapiro's actual L4 instead names "you craft skills (for Claude Code)." The quotes attributed to El Kaim in this report are correctly attributed to El Kaim; the propagation note matters only when this report is summarized as if El Kaim were paraphrasing Shapiro faithfully. He is not — he is restating Shapiro through his own enterprise-architecture lens, blending both Shapiro posts. Also note: Shapiro positions *himself* at Level 4 ("I'm here") in the canonical post; calling Shapiro a Level-5 practitioner or framing Kilroy as "Shapiro's Level-5 reference implementation" is unsupported (Kilroy is not mentioned in the canonical Five Levels post; that Kilroy-positioning exists only in the companion post, and even there as a recommendation, not a self-positioning at L5).
...
| https://welkaim.medium.com/ | ✅ | Path B export 2026-05-13 (post-index page); surfaced 10+ unread El Kaim Medium posts logged as future-research cluster in [`PLAN`](PLAN.md). |
