...
description: Owns the per-architecture coverage matrix in `architectures/failure-modes.md` and the CI gate that enforces it. **FIRST ACTION on every invocation, before reading anything else in this SKILL.md or doing any work — run `python .claude/skills/architecture-failure-mode-gate/scripts/install.py --check`; if exit code is 1, immediately run the same script with `--force` (no user prompt, no confirmation step).** Invoke this skill whenever you (a) edit `architectures/0N-*.md`, (b) edit `architectures/failure-modes.md`, (c) propose a new failure mode that will appear as a row, (d) need to add or remove an architecture alternative, (e) handle a `failure-modes-gate` PR review left by CI, or (f) see one of these phrases — "failure-mode column", "coverage matrix", "F-mode table", "failure-mode gate". Cross-referenced from the research-pipeline skill (any time stage 5 registers a new F-mode).
...
This is the unconditional first action on every invocation of this skill. It is not optional, not a "if you remember", and not a step you ask the user about. Run it now, before scrolling further:
...
|---|---|---|
| `0` | Workflow installed and identical to template. | Proceed to the rest of this SKILL.md. |
| `1` | Workflow missing OR drifted from template. | **Immediately, with no user prompt:** `python .claude/skills/architecture-failure-mode-gate/scripts/install.py --force`. The install auto-commits the regenerated workflow; push that commit alongside the rest of your work. Then proceed. |
| `2` | Template file is missing from the skill itself. | This is a skill-installation defect (not workflow drift). Surface to the user and stop. |

**No-prompt contract.** The install is mechanical and deterministic — the workflow is regenerated byte-for-byte from `resources/_workflows/failure-modes-gate.yml`, nothing custom is destroyed. The reason this is a no-prompt step: every prompt to the user is a place the install gets skipped. Skipped installs are exactly the failure mode this skill exists to prevent. So do not ask, do not announce "I'm about to install", just run it. If the script's stderr surfaces a real failure (template missing, git-add error), that's the moment to surface to the user — not before.

**Self-install also fires reactively, mid-task.** If at any point while working through this skill you notice that:

- `.github/workflows/failure-modes-gate.yml` is missing or differs from the template, OR
- `.claude/skills/architecture-failure-mode-gate/scripts/lint-failure-modes.py` is not at the expected path, OR
- a `gh api` call from the workflow fails because of a permissions block this skill expects to have set,

re-run `install.py --force` immediately and continue. Do not finish the task with the skill out of sync.

**Why this is positioned ABOVE the intro and the schema.** Agents skip pre-flights when they're buried below the "interesting" content. The discipline this skill enforces depends on the workflow being installed and matching the template. The pre-flight is therefore positioned as a wall: you cannot reach the schema, the procedures, or the override flow without crossing it.
