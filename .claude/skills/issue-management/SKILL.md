...
| `2` | Skill-internal file missing (`SKILL.md`, `scripts/install.py`, or either of the two hook templates under [`resources/hooks/`](./resources/hooks/)). | This is a skill-installation defect, not gate drift. Surface to the user and stop. |

**No-prompt contract.** The install is mechanical and deterministic —
each hook script is regenerated byte-for-byte from
[`resources/hooks/require-issue-management-skill.sh`](./resources/hooks/require-issue-management-skill.sh)
and
[`resources/hooks/remind-questions-on-askuserquestion.sh`](./resources/hooks/remind-questions-on-askuserquestion.sh),
and the settings.json edit is an idempotent merge that touches only our
two PreToolUse entries. Every prompt to the user is a place the install
gets skipped; skipped installs are exactly the failure mode this skill
exists to prevent. Do not ask, do not announce — just run it.

**What the gates do.** Once installed, `.claude/settings.json` carries
two PreToolUse hooks:

1. **The hard gate** intercepts every call to `mcp__github__issue_read`,
   `mcp__github__issue_write`, `mcp__github__add_issue_comment`,
   `mcp__github__list_issues`, `mcp__github__search_issues`, and
   `mcp__github__sub_issue_write`. If the per-session marker
   `/tmp/.claude-skill-loaded-issue-management` exists, the call goes
   through. If not, the hook exits 2 with an instructive stderr telling
   the agent to load this skill first. The marker is touched by
   `install.py --check` and `install.py --force`, so a successful
   pre-flight is the load signal.
2. **The `AskUserQuestion` reminder** intercepts every call to
   `AskUserQuestion` when the skill is loaded. It exits 2 with a short
   stderr reminder pointing at the [QUESTIONS behavior](#behavior-questions-asking-the-user-for-clarification):
   any `AskUserQuestion` about an in-flight issue is a `[QUESTIONS]`
   event and the issue-thread comment + `question` label must be
   posted FIRST. To proceed (whether or not the question is
   issue-related), the agent touches the single-use bypass marker
   `/tmp/.claude-skill-issue-management-ask-suppress` and re-calls
   `AskUserQuestion`. The hook deletes the bypass marker after honouring
   it, so the friction recurs on the next call. This is intentional —
   prose-level rules in this file are not sufficient (the mistake
   recurred even while diagnosing it); the hook supplies mechanical
   friction so the rule is considered every time.
...
- **You're about to call `AskUserQuestion` and the question is about
  the in-flight issue.** This is the most-forgotten trigger. The
  question text being ergonomic chat ("Which approach do you want?",
  "What's the scope?", "Should I pick A or B?") does not change the
  rule — the user's answer is part of the durable record of the issue
  and belongs on the issue thread. See the
  [QUESTIONS behavior](#behavior-questions-asking-the-user-for-clarification)
  for the full procedure. The `AskUserQuestion` PreToolUse reminder
  hook makes this mechanical, but the agent is still expected to know
  the rule.
...
in-flight issue. **Includes scope, planning, option-picker, and
direction-confirmation questions — there is no "this is just scoping"
or "this is just lightweight" carve-out.** Each batch of questions gets
one comment — do not post one comment per question.

**`AskUserQuestion` is the most-forgotten surface for this trigger.**
The harness offers an ergonomic chat picker (`AskUserQuestion`) that
feels different from "posting on the issue", but any `AskUserQuestion`
about the in-flight issue **is** a QUESTIONS event. The issue-thread
comment + `question` label go up **first**, then the `AskUserQuestion`
call. The user's answer is part of the durable record of the issue;
chat is not durable. The PreToolUse reminder hook installed by
`install.py` enforces this mechanically — it intercepts every
`AskUserQuestion` call when the skill is loaded and exits 2 with a
stderr reminder. The bypass marker
`/tmp/.claude-skill-issue-management-ask-suppress` is single-use and
must be re-touched before each call; it is intentional friction.
...
4. **If the next step is `AskUserQuestion`**, only now touch
   `/tmp/.claude-skill-issue-management-ask-suppress` and call the
   tool. The hook will let the call through this once; the bypass
   marker is consumed so the next `AskUserQuestion` call re-triggers
   the reminder.
5. If the agent's session is about to end before the user can answer,
...
**Non-issue `AskUserQuestion` calls.** When the question is genuinely
not about an in-flight issue (e.g. a tooling preference, a question
about the user's environment), the agent still has to acknowledge the
hook reminder. Touch
`/tmp/.claude-skill-issue-management-ask-suppress`, then call
`AskUserQuestion`. This is the "I considered it, not issue-related"
branch — the friction is one touch per call, not a wholesale block.
...
| Asked | `[QUESTIONS]` | Agent needs user input — **including any `AskUserQuestion` call about the in-flight issue**; no scoping/lightweight carve-out | Add `question` label | [`templates/comment-questions.md`](./templates/comment-questions.md) |
...
4. if the next step is AskUserQuestion (chat picker):
     touch /tmp/.claude-skill-issue-management-ask-suppress
     call AskUserQuestion
   (the bypass marker is single-use; the hook deletes it after honouring it)
5. if session may end before user answers: record in research/PLAN.md or IN-FLIGHT.md per in-flight-workflow-tracking
```
...
- **Calling `AskUserQuestion` about an in-flight issue without first
  posting the `[QUESTIONS]` comment.** The chat picker is ergonomic but
  not durable; the issue thread is. The user's answer to a scoping or
  planning question is part of the issue's record. The reminder hook
  installed by `install.py` blocks `AskUserQuestion` until the agent
  touches the single-use bypass marker; the bypass is "I have
  considered the rule", not "the rule doesn't apply." Treating scope
  questions, option pickers, or "lightweight" confirmations as carve-outs
  is the failure pattern this convention exists to prevent.
...
- [`scripts/install.py`](./scripts/install.py) — self-installing
  pre-flight; installs and verifies both PreToolUse hooks +
  `.claude/settings.json` entries, and touches the per-session marker
  on every `--check` / `--force` run.
- [`resources/hooks/require-issue-management-skill.sh`](./resources/hooks/require-issue-management-skill.sh)
  — the hard-gate hook script template (matched on issue-touching MCP
  tools). `install.py` copies it to
  `.claude/hooks/require-issue-management-skill.sh` and `chmod +x`s it.
- [`resources/hooks/remind-questions-on-askuserquestion.sh`](./resources/hooks/remind-questions-on-askuserquestion.sh)
  — the `AskUserQuestion` reminder hook script template (matched on
  `AskUserQuestion`). `install.py` copies it to
  `.claude/hooks/remind-questions-on-askuserquestion.sh` and
  `chmod +x`s it.
- [`templates/comment-started.md`](./templates/comment-started.md) — STARTED comment template.
- [`templates/comment-questions.md`](./templates/comment-questions.md) — QUESTIONS comment template.
- [`templates/comment-answers.md`](./templates/comment-answers.md) — ANSWERS comment template.
- [`templates/comment-pr-opened.md`](./templates/comment-pr-opened.md) — PR-OPENED comment template.
- [`templates/comment-closed-no-pr.md`](./templates/comment-closed-no-pr.md) — CLOSED-NO-PR comment template.
- [`templates/comment-duplicate.md`](./templates/comment-duplicate.md) — DUPLICATE comment template.
- [`templates/comment-invalid.md`](./templates/comment-invalid.md) — INVALID comment template.
- [`templates/comment-wontfix.md`](./templates/comment-wontfix.md) — WONTFIX comment template.
- [`templates/comment-sub-issue-linked.md`](./templates/comment-sub-issue-linked.md) — SUB-ISSUE-LINKED comment template.
- [`templates/comment-skeleton.md`](./templates/comment-skeleton.md) — the shared skeleton, copied from
  when modify-behavior mode creates a new per-event template.
- [`spec/FILLING-TEMPLATES.md`](./spec/FILLING-TEMPLATES.md) — concrete guidance on how to fill each
...
- `.claude/hooks/require-issue-management-skill.sh` — runtime hook
  script invoked by the hard-gate PreToolUse hook entry.
- `.claude/hooks/remind-questions-on-askuserquestion.sh` — runtime
  hook script invoked by the `AskUserQuestion` reminder PreToolUse hook
  entry.
- `.claude/settings.json` — gains (or has merged into it) two
  PreToolUse hook entries:
  one matching the issue-touching MCP tools and pointing at the
  hard-gate hook, the other matching `AskUserQuestion` and pointing at
  the reminder hook. Idempotent; other entries are untouched.
