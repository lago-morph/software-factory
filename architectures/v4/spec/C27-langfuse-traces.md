# C27 — LangFuse trace store & browser (`langfuse-traces`)  (Spec, canonical track)
...
- **INV-2 (two separate sinks — the G04 rule).** The OTLP path terminates **here** (LangFuse); the conversation/raw-bodies path terminates at **CXDB via C24**. The two never cross. C27 is the OTLP-sink half of the two-sinks invariant that C25/C26 also assert (README:411–413; AI-CONTEXT:210). Per **D-12**, this rule stays as **cross-referenced per-spec notes** (fork at C25, anti-edge at C26, C24/C27 cross-referencing) — **no new shared subsystem doc**.
