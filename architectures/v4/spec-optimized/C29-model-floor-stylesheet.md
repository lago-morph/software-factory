...
| **L2 `cross-family`** | `independence_class ⩾ cross-family-same-provider` | only if a 2nd family is registered | when a second Anthropic-or-other family is available |
| **L3 `cross-provider`** | `independence_class = cross-provider` (the literal README:189 reading) | only with a 2nd-provider key (DELTA-03 seat) | the strong-isolation eval tier |
...
- **OQ-1 (top, G08/G20 → C32/C34/C50):** Is **L1 `prompt-independent` (same-weights, disjoint-context) judging trustworthy enough** to gate routine in-loop satisfaction, with L2/L3 reserved for promotion? This is the crux Track B forces into the open: if L1 is *not* trustworthy for any decision, then the factory cannot self-evaluate at all on the pure-Max floor and the **judge-seat (DELTA-03) becomes mandatory, not optional** — re-pricing G32. Needs C32/C50 to state the minimum independence level per decision class.
- **OQ-2 (DELTA-03, G20/G37):** Is a metered-API **judge seat** compatible with the project's "no second API key under Max" posture, and where does its credential live (secrets store is G37, unspecified)? The seat is the only proposed second-family path; its admissibility gates L2/L3 entirely.
