...
**`SpecCorrectionRequest` schema (C08 seam — C52 constructs this type on the `spec` route per D-44; consumed by independent authoring path):**
...
| `requested_by` | `string` (`"kind:id"` per D-29) | R | The actor that issued the correction request; MUST be the repair router (C52 rig identity, e.g. `"rig:bootstrap-N"`) — NEVER a worker-rig identity; **C34 is the canonical enforcement owner (D-45)**; E-C08-07 fires as defense-in-depth if a worker-rig actor reaches this guard | C52 stamps its own rig identity; C34 validates (canonical); C08 authoring path entry-guard (defense-in-depth) |
...
| `defect_detail` | `string` | R | The judge's `root_cause_rationale` + `repair_rationale` from `DiagnosisRecord` — the human-readable attribution explaining what the defect is; the authoring path reads this to understand what to fix | C32 writes into `DiagnosisRecord`; C52 copies verbatim; C08 author reads |
| `requested_at` | `timestamp` | R | UTC timestamp when C52 issued the request | C52 writes |
| `spec_ref` | `path` | R | The C08 spec path (`agents/<name>/prompt.template.md`) being corrected | C52 writes; C08 authoring path reads |
...
| Code | Condition | Surfaced-as | Caller recovery |
|---|---|---|---|
| **E-C08-07** | Worker-rig-originated spec-correction request — a `SpecCorrectionRequest` with `requested_by ∈ {rig:worker-*, agent:worker-*}` reaches the C08 authoring path. **Defense-in-depth per D-45: the authoritative check is C34's provenance audit; E-C08-07 is a redundant guard at the C08 boundary.** | The authoring path rejects the request and logs a security violation; no spec change is made; C34 should have already blocked the request before it reached this guard | The request must be reissued from the legitimate repair router (C52 rig identity); investigate how a worker-rig actor bypassed C34's `audit_correction_request_provenance()` — this is an integrity breach (INV-5 / anti-gaming / D-45) |
...
| **AC-C08-09** | Given a `SpecCorrectionRequest` with `requested_by = "rig:worker-1"` (a worker-rig identity); when C34's `audit_correction_request_provenance()` evaluates the `requested_by` field (canonical enforcement, D-45); then the request is REJECTED by C34 before reaching the authoring path; additionally the C08 entry-guard (defense-in-depth, E-C08-07) also rejects it if it somehow passes C34; no spec change is made under either path | **E-C08-07** assertion; INV-5 (anti-gaming); D-43 anti-gaming invariant; D-45 (C34 canonical enforcement; C08 defense-in-depth); OQ-3 RESOLVED |
...
- **OQ-3 — RESOLVED by D-45 (2026-06-02).** The `SpecCorrectionRequest.requested_by` enforcement point is now settled: **C34 is the canonical owner** — it extends its existing `DiagnosisRecord.created_by` provenance audit (E-C34-08) to `SpecCorrectionRequest.requested_by`; a request whose `requested_by` resolves to the implementing worker rig fails the audit and is rejected, never executed. **C52 MUST stamp `requested_by` and MUST NOT route a correction whose C34 audit fails** (consume the audit verdict before hand-off). **E-C08-07 is retained as defense-in-depth** — it fires if a worker-rig request somehow reaches the C08 authoring path boundary, but delegates the authoritative check to C34 (D-45).
