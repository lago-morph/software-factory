...
- **Bias-guard note (added 2026-05-24):** An earlier wording of this entry described the mechanism as "the factory classifies work units into automation-eligible vs human-required *by stakes / risk tier*" — a phrasing that presupposed tier-classification as the factory's mechanism. The Phase-2 axis-divergence audit flagged this as the smoking-gun contamination source for the U-A + U-C tier-axis convergence. The wording above is neutral: F57 names the eligibility-mechanism-drift failure regardless of which axis the architecture uses for the underlying classification.
