...
- **Type-bundle pin** (DELTA-03): the v4 bundle ID `softwarefactory.trajectory.v1` and the set of types v4 registers (e.g. `softwarefactory:RawApiTurn`, `:ToolResult`, `:EventBusRecord`, `:JudgeVerdict`) — *named here, schema-owned by C22*, CI-pinned so cold-agent commands (G17: `gc bd find --type …` analog) resolve.
...
6. **Type-triple pin (DELTA-03, G17):** every turn carries a `{bundle_id,type,version}` registrable in C22's `softwarefactory.trajectory.v1` bundle; an unregistered type is rejected at `AppendTurn`; a cold-agent `Query(type=…)` over a pinned type resolves.
...
8. **Performance contract (G11):** measured p50 append < 1 ms for 10 KB payloads and sub-ms point retrieval on a ≥100 GB store — a *measured* gate, recorded as evidence against the asserted §5.5 claim (full TB-scale is sweep-3).
