...
- **OQ5 (→ review-log, RESOLVED by D-2):** `pack_id` joins one factory-owned reverse-DNS identity scheme. Per D-2, the single root is `softwarefactory.v4` with per-store sub-bundles: `softwarefactory.v4.beads` (C20 bead types), `softwarefactory.v4.trajectory` (C22 CXDB turn types), and **`softwarefactory.v4.packs` (C02 pack ids)**. The earlier divergent roots (`v4.beads.v1`, `strongdm.factory.v4`, `softwarefactory.trajectory.v1`) and vendor `strongdm.*` are dropped. C02 adopts `softwarefactory.v4.packs` as the pack-id namespace so the factory has one identity scheme, not four.
