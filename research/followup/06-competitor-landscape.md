...
- **Microsoft Amplifier** (`github.com/microsoft/amplifier`) — ✅ **resolved 2026-05-16 (Cluster I drain)**: now primary-anchored in `research/28-schillace-sunday-letters.md`. See the "Microsoft Amplifier — now anchored in report 28" section above. Residual open question: read the GitHub repo's SKILL.md / yaml-recipe files directly for the exact schema of a "recipe," and confirm/refute the "session analyst" + "foundation expert" + "Crusty Old Engineer" agent definitions are stored as version-controlled markdown+yaml files in the same way Vincent's Superpowers stores SKILL.md.
...
| `sundaylettersfromsam.substack.com` (Schillace on Amplifier) | ✅ primary (Cluster I drain, 2026-05-16) | All 11 Sunday Letters drained into `research/28-schillace-sunday-letters.md`; Amplifier internals (Dev Foundry, recipes, Context Query, session analyst, foundation expert, Crusty Old Engineer, gene transfer) primary-anchored. |
