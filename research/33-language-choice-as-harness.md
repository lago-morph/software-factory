...
| Report 37 (academic LLM agent collusion) | §5 language-effect on policy | **Empirical extension from code-generation layer to policy / decision layer (added 2026-05-16, Cluster-O drain).** This report's central claim was that *programming-language* choice shapes the **code an LLM agent writes**. Neves & Bussmann document the complementary empirical finding that *natural-language* prompt choice (English vs Portuguese) shapes the **decisions an LLM agent takes**: Gemini-Portuguese neutral-persona agents reach median pricing of $7.10/$7.00 vs Gemini-English $6.80/$6.20 (+$0.30 to +$0.80 median shift; −56% to −66% volatility); Gemini-Portuguese active-persona reaches 100% rounds above the Bertrand-Nash equilibrium vs Gemini-English 94%. **Language affects both layers — code-generation (this report) and policy / decisions (report 37) — and the corpus' language-as-harness thread is therefore two-axis.** Open question: what is the interaction surface (e.g., a Portuguese-prompted LLM agent writing Elixir vs Kotlin code in a multi-agent setting)? Unstudied. |
