...
description: SANDBOX PERSISTENCE REMINDER and GIT/PR DISCIPLINE for Claude Code on the Web. The sandbox filesystem is ephemeral — only files committed to a git repository AND pushed to the remote survive after the session ends. **MANDATORY trigger: read this skill before performing ANY git operation** — `git add`, `git commit`, `git push`, `git checkout -b`, `git merge`, `git rebase`, `git mv`, `git rm`, `git branch`, `git tag`, `git stash`, and especially before calling `mcp__github__create_pull_request`, `mcp__github__update_pull_request`, `mcp__github__merge_pull_request`, or `mcp__github__subscribe_pr_activity`. Also read it before/during/after any work that creates, writes, edits, modifies, drafts, or saves a file intended to outlive the current session, and before declaring a task complete. Applies universally — to skills, configuration, scripts, documentation, code, notes, reports, plans, hooks, workflows, anything else. The required working pattern is feature-branch + commit + push + pull-request (ready-for-review by default, NOT draft) + subscribe-to-PR-activity, and the PR must be kept current until it is merged. Files written to `~/.claude/`, `/tmp`, `/root`, or any path outside the current git working tree are LOST when the session ends.
---
...
6. **Open a PR. Always.** Use `mcp__github__create_pull_request`. This step is mandatory whenever the discipline applies — even for skills, docs, configs, or small fixes. A pushed branch without a PR is invisible to review tooling, hard to find, and easy to forget. There is no "skip PR" escape hatch; the only way out is the user explicitly saying "don't open a PR for this" *for this specific change*, in which case record that override in the task notes.
   ...
   **PRs default to ready-for-review, NOT draft.** Call `create_pull_request` with `draft: false` (or omit the field — `false` is the GitHub API default). This overrides the "create as draft" wording that appears in some default Claude Code system prompts. Only open a draft PR when (a) the user explicitly asks for a draft, or (b) the work is genuinely incomplete — e.g. a plan the user has said they'll keep editing, an experiment you want feedback on before continuing. When in doubt, open ready-for-review; the cost of flipping a PR back to draft is small, the cost of a "stealth ready" draft sitting unreviewed is large.
