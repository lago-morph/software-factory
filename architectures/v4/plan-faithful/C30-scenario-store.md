...
C30 is *adoption (Inspect AI DSL) + a small provider pack + a held-out storage layout + day-0 signing* — not
authorship of a DSL or of isolation enforcement. Per D-13, **C30 stores/authors in the isolated rig**;
**C42 provides** the partition and **C34 enforces+audits**. The work is: adopt + pin Inspect AI, build the
`[[service]] type="inspect_ai"` provider pack (as a C17 tool node), establish the separate scenario repo +
`scenarios/<component>/` layout bound to the `scenarios` partition, add day-0 signing, and **publish the
scenario-path feed C34 audits against** — then freeze the corpus/layout/feed seams the evaluation tier
(C31, C32, C34) builds on.
...
| **T5** Day-0 signing | Sign each scenario at authoring time for tamper-evidence/provenance (AC-5; F9/F7). C30 *signs on store*; verification is C34/baselining; **key custody is G37/C03** (note, don't build). | M | T3 |
| **T6** Scenario-path feed to C34 | Publish the corpus layout/paths (+ signatures) as the feed **C34** enforces+audits actual implementer reads against (AC-7; README:173). **C30 publishes; C34 enforces+audits (D-13).** | M | T3, T4 |
| **T7** Freeze corpus/layout/feed seams | Enumerate + freeze I1–I7 (Task DSL, repo+layout, provider pack, partition/rig binding, day-0 signature, path-feed, corpus retrieval) + the §4 scenario-record [FAITHFUL-FILL] so C31/C32/C34 build against stubs. | M | T2–T6 |
...
- **Upstream of C30:** **C17** (the tool-node abstraction the Inspect AI provider pack is wrapped over —
  ...
  C03 (secrets) is referenced by T5 for key custody but is *not* a build prereq (signing key is G37,
  deferred).
...
- **Downstream gated by T7 (seam freeze):** **C31** (runner → I1 Task DSL + I7 corpus retrieval; also needs
  ...
  D-1), **C34** (holdout enforcement+audit → I6 path-feed + I5 signatures; **enforcement is C34**), and the
...
- **Storage track (T3→T4→T5):** stand up the separate repo + layout, bind to the `scenarios` partition, add
  day-0 signing. T5 (signing) and T4 (partition binding) are independent of each other once T3 (repo) exists.
- **Convergence (T6, T7):** the C34 path-feed (T6) needs both the layout (T3/T4) and signatures (T5); the
  seam freeze (T7) needs all of T2–T6.
- **Cross-component:** **C31's** runner design (against the Inspect AI Task DSL + I7) and **C34's**
  enforcement/audit design (against the I6 path-feed + I5 signatures) can proceed in parallel with C30's
...
- **M4 — Scenario-path feed + signature frozen (T6/T5/T7):** the path/label feed (I6) + day-0 signature
  format (I5) → unblocks **C34** (enforcement+audit) and the F7 baselining path. *Signing key custody is
  G37/C03 — flagged, not in this freeze.*
...
4. **G37 — signing-key custody.** Day-0 signing (F9) needs a key; v4 has no secrets story (plaintext
   `city.toml`/env). C30 *signs*; key custody defers to C03's SecretResolver (OQ-3). Until then the
   signature's assurance is capped — flag, don't block. *(G37 ≠ FE-3 per D-14.)*
...
- The **separate scenario git repo** stands up with the `scenarios/<component>/` layout, read-only-from-
  implementer perms, bound to the **`scenarios` partition / `scenario_authoring` rig** C42 provides;
  scenarios are **day-0 signed**; the corpus is versioned + append-growing. (AC-2…AC-6, AC-8.) **Resolves
  ...
  C43 (D-13).**
- The **scenario-path feed** (+ signatures) is published so **C34** can enforce+audit actual implementer
