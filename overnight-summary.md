...
- **0 morning-review items remaining.** The BF-L / U-B asymmetry rule was approved by the user 2026-05-25 and codified in [`candidate-registry.md` § Phase-3.5.5 RG-primitive rule](./architectures/v3/candidate-registry.md#phase-355-rule-on-load-bearing-rg-primitives-binding-user-approved-2026-05-25). BF-L's RG views default to accept-as-RG unless BF-L opts in to a bounded sub-track at Phase 4 entry. The user also approved a new AGENTS.md rule that adversarial reviews MUST be real subagents (no more inline-simulated reviewers).
