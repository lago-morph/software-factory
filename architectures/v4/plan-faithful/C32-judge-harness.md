...
| T6 | **Multi-judge ensemble.** Run N scorers over one trajectory; reduce to a disagreement signal in the `ScoreRecord` (README:187; F46) — Inspect AI provides multi-scorer; C32 owns the request+reduce policy. | M | T3, T4 |
| T7 | **Judge rig placement (independence-by-isolation).** Run the judge in a **separate `judge` rig** with a disjoint role/prompt/rubric from the coder; stamp the active independence level on each record (D-1/D-13). | S | C42 |
