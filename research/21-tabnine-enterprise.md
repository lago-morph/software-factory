...
**Round / cluster:** Round 5, Cluster 13.1.4, per [`PLAN`](PLAN.md) §13.1.4.
**Stance:** *Tabnine's enterprise positioning is the closest existing-product analog to the governance discipline that [`10-governance`](followup/10-governance.md) (Round-3 §11.10) and [`16-el-kaim-book-council-and-delegation`](16-el-kaim-book-council-and-delegation.md) (Round-4 L1–L4) argue any regulator-defensible factory needs. Treated as a product specification rather than as marketing, it supplies four primitives — private deployment, governed context, typed instructions, and per-output provenance — that the four architectures of this corpus do not currently name as first-class.*
...
**Gap vs. El Kaim and Foundry — sharpened by primary source:** Tabnine guidelines are *prompts*, not *contracts*. No machine-checkable invariant, no decision-of-origin attribution, no failure-by-level allocation, no schema validation. A guideline can drift, get longer (up to and past the 500-line guideline), contradict an earlier guideline, and the system does not flag the contradiction — the **G7 intent drift** failure mode in [`10-governance`](followup/10-governance.md) §3. The admin-UI precedence rule is structural (admin > personal), but no precedence ordering is documented *among* admin guidelines themselves. Tabnine has done the *plumbing* (multi-scope, named kinds, admin-administered, admin-precedence) without the *typing*: structurally sympathetic but incomplete for a regulator-defensible factory.
...
**Lethal-trifecta lens** (Simon Willison's framing per [`05-simon-willison`](05-simon-willison.md)): exfiltration risk requires (1) access to private data, (2) exposure to untrusted content, (3) an outward path. Tabnine's posture attacks the trifecta unevenly:
...
*Sources: T1–T10 (see §0 source-status table for URLs and access status), all probed 2026-05-11 and drained from `research/fetched/issue-27/` on the same day. Context: [`10-governance`](followup/10-governance.md), [`16-el-kaim-book-council-and-delegation`](16-el-kaim-book-council-and-delegation.md), [`03-phase-gated-foundry`](../architectures/03-phase-gated-foundry.md), `reference-only/chatgpt-deep-research-2026-05-11/sources.md`.*
