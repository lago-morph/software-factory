...
| **T3** Provision LangFuse credentials (G37, minimal) | Supply LangFuse's required secrets — DB password, project/public+secret API keys, initial admin — via `city.toml`/env per the existing plaintext convention (AI-CONTEXT §13.2). **Note the G37 gap; build no secrets store** (FE-3); record the deferral (spec §7, OQ-4). | S | T1 |
...
3. **G37 plaintext credentials — noted, not solved.** Risk: scope-creep into building a secrets store, or shipping creds insecurely. Mitigate by **referencing creds from `city.toml`/env per convention and explicitly deferring** to FE-3 (spec §7, OQ-4) — do **not** build a secrets layer v4 doesn't have (it fails the capability-for-principle bar). Record the deferral for the reviewer.
