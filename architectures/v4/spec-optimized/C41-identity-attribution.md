...
- **Depends on:**
  ...
  - **C43** (isolation boundary) — soft/co-foundational for `boundary_class` (DELTA-07): C43 defines the boundary taxonomy; C41 stamps every attribution with the class touched. This is a *vocabulary* dependency, not a runtime blocker (C41 can label `production` from Phase 0 even before twins exist — which is the point: it makes G31's exposure window auditable).
...
- **Tamper-evidence (DELTA-04):** each actor's attributions form an append-only hash chain anchored in C23; any retroactive edit/deletion breaks the chain and is detected by `verify_chain` (makes F14 *detectable*, not merely "Addressed").
...
| `attested` | `signed` **plus** an independent co-signature (e.g., the rig + the city, or a second-family attestor) — for the highest-stakes actions. | two signatures | Self-modifying / RSI-relevant actions (F43/G35), L5 auto-ship fix-tasks. |
...
- **G36 (attribution integrity optional/deferred — RESOLVED):** signing is graduated-mandatory and default-on for security-relevant actions (DELTA-01); verification is a first-class service (DELTA-05); the key model is specified (DELTA-06); history is tamper-evident (DELTA-04). The *presence* of an actor is mandatory (co-owned with C19). Attribution is no longer self-asserted where it matters. Residual: the policy tier defaults are an operator decision (OQ1); secrets *storage* for keys is delegated (G37/OQ2).
...
- **Cost / scale:** `asserted` is free (the common intra-rig case); `signed` is one signature/verify (cheap, cross-boundary only); `attested` (two signatures) is reserved for rare high-stakes actions. The chain hash is O(1) per attribution. No external service required in Phase 0 (keys + signatures are local). Cost scales with cross-boundary action volume, not total action volume — a deliberate tiering choice.
...
2. **Graduated-mandatory signing (DELTA-01 / G36 integrity):** a cross-`boundary_class` action or a C06 Mail carrying only `asserted` is rejected under the default policy tier; the same action with a valid `signed` attribution passes. (Policy-tier conformance test.)
