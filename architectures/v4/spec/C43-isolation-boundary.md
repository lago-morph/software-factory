...
| **F31** Substrate safety floor = weakest adapter (F-MODE §6, "Addressed") | The safety floor of the agent substrate. | **Adjacent (owner: C29/C28 single-adapter choice).** Addressed by "single-adapter choice (Claude Code only)" — *not* C43's mechanism. Listed only to disambiguate: C43 is the *blast-radius* boundary, not the *adapter-floor* declaration. |
