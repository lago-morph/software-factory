...
| **T3** Boot single worker | Run `gc start --foreground` (under `tini` in container). Verify `gc status` shows controller running and worker pane in `gc session list`. Execute the minimal stand-up smoke signal: `bd create "conformance-smoke-signal"` from `/workspace/rigs/rig1`. | S | T2, C28 (the `claude` binary, pre-staged). Note: C04 is the owning spec for the session model but does NOT need to be built before `gc start` uses the native tmux provider — tmux is a Gas City built-in, not a v4 component (F7). | AC-C01-1 |
