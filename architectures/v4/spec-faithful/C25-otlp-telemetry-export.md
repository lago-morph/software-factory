...
- It is **not** CXDB. CXDB (C21) has **no native OTLP receiver** (AI-CONTEXT:210) — so the OTLP channel does *not* and must *not* terminate at CXDB; only the raw-bodies channel feeds CXDB, and only via the C24 bridge (this is the crux of G04 — see §6/§9).
...
| Lateral (sink it must NOT hit) | **C21** CXDB | CXDB has no OTLP receiver (AI-CONTEXT:210); the OTLP channel must terminate at C26, never directly at CXDB. The CXDB-bound data goes only via the raw-bodies → C24 path. |
...
- **Correlation attributes** carried on the signals (AI-CONTEXT:178): `prompt.id`, `session.id`, `user.account_uuid`, `organization.id`, `terminal.type`. **`session.id` is the load-bearing correlation key** for the CXDB parent-chain (AI-CONTEXT §5.4 line 229 "parent-chain via `session.id`") — C25 must guarantee it is present on the raw bodies / signals so C24 can build CXDB turn lineage.

### 3.4 Invariants

- **INV-1 (two parallel sinks, never crossed).** The OTLP channel terminates at C26 (the collector); the raw-bodies channel terminates in the on-disk dir for C24. These are **two separate sinks** (README:411 `OTLP| OTel`, line 413 `raw bodies| Bridge`). The OTLP channel is **never** wired to CXDB (CXDB has no OTLP receiver, AI-CONTEXT:210). *(This invariant is the resolution of G04 — see §6/§9.)*
...
- **INV-3 (`session.id` present on CXDB-bound data).** Every raw-body artifact the escape hatch writes carries enough identity — at minimum `session.id` (AI-CONTEXT:178, 229) — for C24 to derive the CXDB parent-chain. C25's contract guarantees the correlation attributes are emitted; the *mapping rule* to CXDB's parent pointer is C24's (G26).
...
|---|---|---|
| **G04** (CXDB vs OTel framing tension) | A naive integrator wires the OTLP channel into CXDB — the explicitly-rejected path (AI-CONTEXT §11.3 / line 210). | **Resolved by INV-1:** C25's contract states the OTLP sink is **C26 only**; CXDB is fed **only** by the raw-bodies → C24 path. The spec names the two sinks explicitly (the thing G04 says v4 "never stated"). See §9 OQ-1. |
...
6. **AC-6 (two sinks, never crossed).** The OTLP stream is observed terminating at C26 (→ C27) and the raw bodies at the C24 dir; **no OTLP is sent to CXDB** (INV-1; AI-CONTEXT:210). This is the G04 acceptance check.
...
- **OQ-1 (→ [review-log](../_meta/review-log.md), top open question) — G04 two-sinks statement.** v4 is consistent but never says "two separate sinks" out loud, so a naive integrator may wire OTLP → CXDB (the rejected path, AI-CONTEXT §11.3 / line 210). **Faithful resolution adopted:** INV-1 + AC-6 state explicitly that the OTLP channel terminates at C26 only and CXDB is fed *only* by raw-bodies → C24. Open item for the reviewer: should this two-sinks rule be hoisted into a shared Observability-subsystem note (C25/C26/C24) so all three specs assert the same boundary, rather than living in C25 alone? (Reading A: state once in C25 as the source; Reading B: restate in each. Faithful pick: state in C25 — the source — and cross-reference from C24/C26, since C25 is where the split originates.)
