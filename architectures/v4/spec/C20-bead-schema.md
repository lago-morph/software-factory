...
> **INT-2 FIX (Sweep-2) — enum reconciliation:** D-40's verbatim text says `status: in_progress → completed`, but `completed` is not a member of the envelope's `{open, in_progress, closed}` enum (§4.5.0). This contradiction is resolved by aligning the terminal to the envelope: the `factory_build` terminal state is **`closed`** (not a new `completed` value). The go/no-go outcome — what D-40 intended `completed` to carry — lives in the separate **`milestone_verdict`** field (`"go"` | `"no_go"`) written by C53. This matches the `closed_build` node in §5.1's state diagram (which the diagram already annotates as `status=closed`). The resume/cold-start query reads `--type factory_build --status in_progress`; a finished build has `status=closed` + `milestone_verdict` set. All uses of `status=completed` or `enum{in_progress, completed}` in this spec and in C52/C53 are superseded by `status=closed` + `milestone_verdict`.
...
| `status` | `enum{in_progress, closed}` | R | D-40 lifecycle state, aligned to the envelope `{open, in_progress, closed}`. `in_progress` on build-start (C52 `emit_spec`); **C52 advances to `closed`** after Phase B deploy (`advance_to_closed`). The go/no-go outcome lives in `milestone_verdict`, not in this field. The canonical resume query is `gc bd find --type factory_build --status in_progress` (D-40, XC-2 resolution). | C20 defines slot; **C52 drives transition** |
...
> **Three-writer seam (D-40 + C53 §3.4 NEW SEAM):** One bead, three sequential append-writes: **(1)** C51 writes `transfusion_verdict` + `exemplar_set` at acceptance-time; **(2)** C52 writes `gate_decision` + `workflow_handle` and advances `status` from `in_progress` to `closed` after Phase B deploy; **(3)** C53 writes `milestone_verdict` + `milestone_evidence` + `milestone_decided_at`. All on the same `factory_build` bead. Grain = one decision per factory-built component build. The go/no-go build outcome is `milestone_verdict` (`"go"` | `"no_go"`); `status=closed` is the envelope terminal (INT-2 alignment). This seam is registered to the **orchestrator ledger** as a C53-originated new obligation on C20 (C53 §3.4 "NEW SEAM").
