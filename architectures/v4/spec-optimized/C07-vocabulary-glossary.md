...
- `CanonicalReading` (DELTA-02) — for an **overloaded** term, the pinned sense plus the *rejected* sense(s): `{ term, pinned_sense, disambiguation_rule, rejected_senses[], rename_recommendation }`. This is the authority artifact that resolves G01/G02.
...
- **Overloaded-term ledger** — the set of `CanonicalReading`s. Seeded at sweep-1 with the two known offenders:
  ...
  | **"phase"** (G02) | The **four-phase delivery plan** P0→P1→P2→P3+ (README Part 6; owned by C54). There is **no Phase 4/5/6**. | Stray "Phase 6" (README:342) is a **typo for the numbered "Layer 6"** principle tier, not a missing phase. | "Phase N" with N∈{0,1,2,3} = delivery plan (C54). Any "Phase ≥4" reference is off-canon and MUST be rewritten to either "Layer N" (principle tier) or the correct phase. |
