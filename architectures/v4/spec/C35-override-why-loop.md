...
> Binding decisions obeyed: **D-3** (C20 *authors* the `override` bead-type schema; C35 *logs* `override` beads using it, and does not define the schema). **D-6** (canonical track; no Track-A/B framing). **D-1** (judge/rubric runs on the same provider/family — relevant to the §3 rule-conversion handoff).
...
- **Rule conversion** — turning a surfaced recurring pattern into a **new validation rule** that becomes a C10 (spec linter) / C15 (workflow linter) check, and — where the override class is about *work satisfaction* rather than spec/workflow structure — a new Inspect-AI rubric in the C30 scenario store (README L216).
...
| Upstream (rule sink for satisfaction-class overrides) | **C30** Scenario store | Receives a new Inspect-AI rubric when a surfaced override class is about work-satisfaction rather than spec/workflow structure (README L216). C30's read-isolated rig owns rubric authoring. |
...
| Downstream (rule sinks) | **C10** spec linter, **C15** workflow linter | Receive the *new validation rules* C35 produces (brief: "validation rules C35 produces feed C10 / C15"). C35→C10/C15 is a rule-registration handoff, not a code dependency. |
...
6. **Rule-conversion / rule-emission contract** — for an operator-approved recurring pattern, C35 emits a **new validation rule** to the appropriate sink:
   - spec-structural class → a new **C10** (EARS/INCOSE) rule,
   - workflow-structural class → a new **C15** (Mammoth) rule,
   - work-satisfaction class → a new **Inspect-AI rubric** in **C30** (README L216).
...
6. **Convert.** For an approved pattern, C35 emits a new rule to the right sink — C10 (spec-structural), C15 (workflow-structural), or a C30 Inspect-AI rubric (satisfaction-class). The rule then prevents/flags that override class going forward, closing the loop.
...
| **F52** "More controller patches" / runaway loop | Rule conversion that auto-shipped could spawn rules that cause further overrides (oscillation) — the self-modifying-control trap | Mitigated by **I4 + operator gate** (README L216): conversion is human-reviewed, not auto-promoted, so the loop cannot silently feed itself. |
...
- **AC4 (rule conversion handoff):** an operator-approved recurring pattern is converted into a new rule registered in the correct sink — C10 (spec-structural), C15 (workflow-structural), or a C30 Inspect-AI rubric (satisfaction-class) — and the conversion is reachable **only** through the operator-review gate. (README L216; I4.)
...
- **OQ4:** Rule-conversion encoding per sink — the concrete rule/rubric format for C10 (EARS/INCOSE), C15 (Mammoth), and C30 (Inspect-AI) is each owned by the sink; C35 must target three different rule schemas. Confirm the handoff format with each sink at sweep-2.
