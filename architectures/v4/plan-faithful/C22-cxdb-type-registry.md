...
| T4 | Author the **`softwarefactory.v4` bundle skeleton** — bundle identity + viewpoint set + placeholder type slots — the concrete artifact that closes G17 for the CXDB side. Per-type schemas are stubs at this sweep. | M | T3 |
...
- **WS-C (artifact):** T3 bundle format → T4 the `softwarefactory.v4` bundle.
...
3. **M3 — `softwarefactory.v4` bundle identity + viewpoint enum (T4 skeleton).** Lets downstream pick
...
1. **G17 closure (highest value):** confirm the `softwarefactory.v4` bundle (T4) actually covers the
...
- **Per-component:** the `softwarefactory.v4` bundle exists and is registrable under CXDB `registry/`
