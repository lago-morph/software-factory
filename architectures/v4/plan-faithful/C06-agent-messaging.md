...
| T3 | **Attribution pass-through.** Verify `created_by` is stamped by the substrate on every **Mail** (native bead/event-class stamp) and **confirm against real `gc` whether the ephemeral Nudge carries the same stamp** (concept-6→P9 faithful reading, I3 [FAITHFUL-FILL]; if `gc` proves Nudge un-stamped, record as a faithful gap — do **not** build a C06 stamping path). C06 adds no anonymous path and **no identity machinery** (inherited from C41/P9). | S | T1, C41 |
