...
| Upstream (config) | **C03** Config / feature-flags | Section-presence enables Orders (the `orders` block); off in Phase 0, on in Phase 3b (AI-CONTEXT §3.4; C03 model). |
| Tightly-coupled consumer | **C39** Fix-task generation & loop-closure | The canonical workflow an Order drives in P11: a crash/anomaly event triggers an Order that drives the fix-task → resolution chain (README §3b; component-inventory C39). C40 supplies the *durable event-triggered carrier*; C39 owns the fix-task bead chain + termination/escalation (G35). |
