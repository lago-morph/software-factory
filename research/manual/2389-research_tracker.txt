...
Create new...
All issuesAll pull requestsAll repositories

You have unread notifications(g then n) gGthen nN
...
Watch2 (2)
 ...
 Fork
 2
 Fork your own copy of 2389-research/tracker
...
tui: cap partial text height to prevent streaming blob from pinning t���
Open commit detailssuccess
8 hours agoMay 15, 2026

694e9dd �� 8 hours agoMay 15, 2026
...
2 days agoMay 13, 2026

cmd

cmd

fix: restore pre-PR validate path for .dip/.dot

3 days agoMay 12, 2026

docs

docs

merge main + address CodeRabbit outside-diff items

3 days agoMay 13, 2026

examples

examples

fix(workflows): make Start/Done passthroughs in built-in workflows (r���

yesterdayMay 14, 2026

internal

internal

fix: address CodeRabbit review feedback on PR #209

4 days agoMay 11, 2026

llm

llm

fix(google): address PR #206 review ��� flush helper, defensive clear, ���
...
fix(pipeline/handlers): manager_loop ctx-cancellation race (#227)

9 hours agoMay 15, 2026

testdata

testdata

feat(pipeline): _TRACKER_ROUTE= reserved sentinel + route_required (c���
...
chore: ignore the entire .claude/ directory

last monthApr 20, 2026

.goreleaser.yml

.goreleaser.yml

feat: add Homebrew tap publishing and fix go install version display
...
fix: address PR #88 CI + review ��� sync paths, resolve workDir, error ���

last monthApr 16, 2026

.pre-commit-config.yaml

.pre-commit-config.yaml

build: rename tracker binaries and tooling

2 months agoMar 6, 2026

ARCHITECTURE.md

ARCHITECTURE.md

docs: arch-tree hygiene ��� move context-flow + manager-loop, fix drift

3 weeks agoApr 23, 2026
...
feat(pipeline): activity.jsonl integrity hardening (closes #213) (#223)

2 days agoMay 13, 2026

CRITIQUE_FILES_SUMMARY.txt

CRITIQUE_FILES_SUMMARY.txt

docs: complete dippin-lang feature parity gap analysis

2 months agoMar 23, 2026

Makefile

Makefile

fix(manager_loop): polish bundle ��� colon-key rejection, nil-config gu���

3 weeks agoApr 22, 2026
...
test: harden examples and finalize doc polish

3 weeks agoApr 21, 2026

go.mod

go.mod

chore(deps): bump dippin-lang v0.24.0 ��� v0.25.0 (#225)

2 days agoMay 14, 2026
...
feat(api): tracker.Result.BundleIdentity for library callers
...
fix: remove MostRecentRunID stderr side effects and route warnings vi���
...
feat(cli): Bundle header in tracker audit
...
fix: address PR #90 review ��� auto-enable git artifacts, resolve bundl���

last monthApr 16, 2026

tracker_bundle_test.go

tracker_bundle_test.go

fix: gofmt tracker_bundle_test.go
...
fix: merge main + address PR #209 review feedback
...
fix(cli): doctor accepts .dipx + suppress bundle-irrelevant DIP126 lint
...
refactor(tracker): library API hardening for v1.0

last monthApr 17, 2026
...
fix: address PR #113 round-4 review feedback
...
tracker_examples_test.go

tracker_examples_test.go

docs: isolate ExampleDoctor workdir with temp directory
...
fix: address PR #88 review ��� scanner.Err, doc precision, ResumeRunID ���

last monthApr 16, 2026

tracker_resolve_test.go

tracker_resolve_test.go

fix: detect Windows backslash paths in isExplicitFilePath
...
fix: address PR #135 round-3 Copilot feedback
...
What's new in v0.28.2 (2026-05-14): patch release fixing a runaway-agent bug in three of the four built-in workflows (#230). ask_and_execute, build_product, and build_product_with_superspec defined Start / Done as agent nodes with prompt: Initialize pipeline. and prompt: Pipeline complete. ��� which caused ensureStartExitNodes to skip the passthrough handler and turn them into real codergen sessions with full default tool access and no max_turns cap. A real build_product run was observed spending ~10 minutes and ~39k output tokens inside Start, implementing an entire separate Go project from a SPEC.md found on disk, before getting context canceled. Dropping the prompt lines makes Start/Done passthroughs, matching deep_review.dip which was already correct. Engine-policy follow-ups (default max_turns cap, cancel���retry mapping, runaway-node detection in tracker diagnose, tool-call arg logging, token accounting) remain tracked on #230. No engine changes; no breaking changes.
...
Previously in v0.20.0 (2026-04-21): stack.manager_loop supervisor handler (Attractor spec 4.11); engine-level steering channel; accurate cost estimation via catalog with cache-token pricing; April 2026 model catalog refresh (Claude Opus 4.7, GPT-5.4 family, Gemini 2.5 GA, Gemini 3.1 pro preview); ACP sandbox hardening against .. path traversal. See CHANGELOG.md and docs/architecture/handlers/manager-loop.md.
...
agent
box
LLM agent session (codergen)

human
hexagon
Human-in-the-loop gate (choice, freeform, or hybrid)
...
subgraph
tab
Execute a referenced sub-pipeline

manager_loop
house
Managed iteration loop
...
��� (dim)
Skipped ��� pipeline took a different path
...
v
Cycle log verbosity (all / tools / errors / reasoning)

z
Toggle zen mode (full-width log, sidebar hidden)
...
Enter
Drill down into the selected node (Esc exits)

y
Copy the visible log to the clipboard

Ctrl+O
Toggle expand/collapse tool output

Ctrl+S
Submit human gate input

Esc
Cancel (empty) or submit (with content)

PgUp/PgDn
Scroll review viewport (plan approval)
...
Missing API keys
tracker setup or export env vars

TestMilestone instantly escalates
Stale fix_attempts counter
rm .ai/milestones/fix_attempts

Node fails with no visible error
Tool stderr not surfaced
tracker diagnose shows full output

Pipeline loops forever
Unconditional fallback to loop target
Ensure fallbacks go to an exit node (Done, escalation gate), not back into the loop

Tool retries same error 5 times
Deterministic command bug
tracker diagnose flags identical retries ��� fix the command in the .dip file

Every milestone needs fixing
known_failures has comments or bad format
Ensure bare test names only, no comments ��� v0.11.2 strips them automatically

Build loop skips all milestones
Milestone headers don't match expected format
Use ## Milestone N: Title format ��� v0.11.2 is flexible + fails loudly
...
A human gate fires ��� tracker POSTs a WebhookGatePayload to --webhook-url.

Your service receives the payload, routes it to a human (Slack message, email, etc.).

The human responds ��� your service POSTs a WebhookGateResponse to the callback_url field.

Tracker resumes the pipeline with the human's answer.
...
--webhook-auth
(empty)
Authorization header on outbound POSTs
...
������ Stability note (pre-v1.0): tracker's library API is usable now, but
