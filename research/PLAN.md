...
### 5.2 External-synthesis artifacts (`research/external-syntheses/`)

A separate kind of artifact: synthesis documents produced *outside* this project's research pipeline (e.g., a one-shot ChatGPT deep-research run over the same seed inputs) that we preserve verbatim as **counterfactual** and **quality-control** inputs, not as primary sources. These live under `research/external-syntheses/<short-slug>-<date>/` with a `README.md` orientation file alongside the artifact.

Current contents:

- `research/external-syntheses/chatgpt-deep-research-2026-05-11/` — paired artifact: `report.md` (ChatGPT deep-research synthesis output) + `sources.md` (verified URL bibliography that decodes the report's internal `citeturnXX` citation format) + `README.md`. Catalogued in §13 for source harvest, QC checklist, and synthesis-time counterfactual comparison.

These artifacts do not get drained by Phase 0 (they're not source files to incorporate). They are referenced from PLAN.md §13 (and from any future cross-round synthesis pass).
...
## 13. Round 5 — External-synthesis harvest and comparison

**Status:** Catalog only — none dispatched. Consolidated 2026-05-11 from a user-supplied paired artifact filed at `research/external-syntheses/chatgpt-deep-research-2026-05-11/`.

**Source.** Three files at `research/external-syntheses/chatgpt-deep-research-2026-05-11/`:

- `report.md` — a ChatGPT deep-research synthesis output (174 lines, 6 H2 sections).
- `sources.md` — the verified URL bibliography that decodes the report's internal `citeturnXXviewYY` citation format. 28 sources total: 2 user-uploaded local files + 26 external sources with canonical URLs, types, relevance notes, and an explicit "Weak or missing citations" QC section.
- `README.md` — orientation file explaining how the pair works (the report's citation tokens are ChatGPT-conversation-internal; `sources.md` is the decoder ring).

The deep-research report was generated by ChatGPT from only the seed files `spec-driven-ai-dev.md` and `initial-sources.md` from this repo's root. It did **not** see our multi-round `research/0N-*.md` reports. So it represents what an external one-shot deep-research process produces from the same starting inputs we used to begin Round 1 — making it a counterfactual against our multi-round skill-driven approach.

**Round 5 has three distinct work products.** Each is summarized below as its own subsection. Unlike Rounds 1–4, these are not all "go read a source and write a report" — they include a synthesis-time process input and a deliberate comparison step.

### 13.1 Source harvest — six clusters of primary sources our reports underweight

`sources.md` lists 26 external sources. Roughly half are well-covered in our existing reports (StrongDM, Every, Simon Willison, Anthropic engineering, OpenHands). The other half surface six clusters of primary sources we have **not** systematically covered, each of which warrants its own report. Each cluster below is a dispatch-ready brief.

#### 13.1.1 Cluster — OpenAI Codex substrate audit

**Source IDs from `sources.md`:** S8 (overview), S9 (harness-engineering blog), S10 (AGENTS.md), S11 (Subagents), S23 (Agent approvals & security). The "Weak or missing citations" section also implies the App Server engineering writeup should be cited directly.

**Applicability rationale.** OpenAI Codex is one of the two dominant commercial coding-agent substrates (the other being Claude Code, which is covered in report 04 indirectly). Our corpus has zero direct primary-source coverage of Codex. The five official pages together describe Codex as a substrate (CLI / app / cloud / SDK / App Server), its instruction-layering model (AGENTS.md), its orchestration primitives (Subagents), and its safety controls (approvals, sandboxing, network rules, isolated cloud containers). The harness-engineering blog post is particularly load-bearing — it reports ~1,500 merged PRs in five months in a ~1M-line repository driven by a small Codex-heavy team, the strongest first-party productivity claim in the OpenAI literature.

**Target report filename:** `research/18-openai-codex-substrate.md`

**Expected length:** 2000–3000 words.

**Suggested subagent brief.**
> Read the five OpenAI sources (S8–S11, S23 from `research/external-syntheses/chatgpt-deep-research-2026-05-11/sources.md`) plus the App Server engineering writeup if accessible. Compare against `research/11-openhands-substrate-audit.md` (the analog for OpenHands) and `research/04-every-skill-libraries.md` (which touches Claude Code skills). Extract: (a) the five-surface deployment model (CLI / app / cloud / SDK / App Server) and where each lives in a software-factory architecture; (b) the AGENTS.md instruction-layering model in full, diffed against our project's `.claude/`-based equivalents; (c) the Subagents orchestration primitive and how it composes with the cloud surface; (d) the approvals/sandbox/network policy and what it implies for the lethal-trifecta defense (report 08 §11.8); (e) the harness-engineering productivity numbers verbatim, with the team-size and codebase-size context. Output `research/18-openai-codex-substrate.md`. Final section: "Substrate posture per architecture" mapping Codex's deployment surfaces to each of `architectures/0N-*.md`.

#### 13.1.2 Cluster — GitHub Copilot cloud agent

**Source IDs:** S3 (cloud-agent overview), S24 (changelog: research, plan, code workflow). The "Weak or missing citations" section also identifies code-review and Autofix/CodeQL pages as needing direct citation.

**Applicability rationale.** GitHub Copilot cloud agent is the third major commercial substrate. It runs in ephemeral GitHub Actions environments, does its own planning and self-review, and produces PRs back to the repo — i.e., it's a CI-shaped substrate with the same "agent runs in isolated env, produces a PR, human reviews" loop our Architecture 1/3 patterns describe. This is the closest commercial product to our software-factory thesis. The §11.6 Round-3 competitor-landscape thread mentions Copilot only in passing; this report would do a full substrate audit. Note overlap and dispatch alongside §11.6.

**Target report filename:** `research/19-github-copilot-cloud-agent.md`

**Expected length:** 1500–2500 words.

**Suggested subagent brief.**
> Read S3 (cloud-agent overview) and S24 (changelog), plus the GitHub code-review and Copilot Autofix / CodeQL official docs flagged in `sources.md` §"Weak or missing citations". Compare against report 11 (OpenHands), the §11.6 competitor-landscape thread, and `architectures/01-spec-driven-refinery.md` / `architectures/03-phase-gated-foundry.md` (the two architectures closest to the CI-shape Copilot cloud agent uses). Extract: (a) the ephemeral-GitHub-Actions-environment execution model — what's inside the sandbox, what's not; (b) the research → plan → code → self-review → PR workflow shape; (c) the integration / memory / hooks model — how state survives across runs; (d) the Autofix + CodeQL review pipeline and how it integrates with the agent's own self-review; (e) the human-in-the-loop touchpoints (plan approval, PR review). Output `research/19-github-copilot-cloud-agent.md`. Final section: where does Copilot cloud agent fit on the §11.6 competitor map?

#### 13.1.3 Cluster — Replit Agent (current docs, not the historical Ghostwriter blog)

**Source IDs:** S16 in `sources.md` is the *old* Ghostwriter blog; the "Weak or missing citations" section explicitly says the current Replit Agent docs are the right primary source. The cluster should follow those updated docs (Replit Agent overview, `replit.md` instruction-file docs, integrations docs, skills docs).

**Applicability rationale.** Replit Agent is a fourth commercial substrate — single-machine, app-generation-focused, with native deployment, connectors, and a skills layer. Distinct shape from Codex / Claude Code / Copilot cloud agent because deployment is in-loop. Zero primary-source coverage in our reports.

**Target report filename:** `research/20-replit-agent.md`

**Expected length:** 1500–2200 words.

**Suggested subagent brief.**
> Locate the current Replit Agent docs (the user-facing landing + `replit.md` docs + integrations docs + skills docs — per `sources.md` §"Weak or missing citations" S16's correction note). Do NOT use the Ghostwriter blog as the primary source. Compare against report 18 (OpenAI Codex), report 11 (OpenHands), and the §11.6 competitor map. Extract: (a) the app-generation-as-first-class-output shape vs. Codex/Claude/Copilot's repo-modification shape; (b) `replit.md` as instruction-file substrate, diffed against AGENTS.md and CLAUDE.md; (c) the in-loop deployment model — what's different when the deploy target is built into the substrate; (d) connectors and skills as architecture-layer primitives. Output `research/20-replit-agent.md`. Final section: implications for any of our four architectures that have an "operate the deployed system" loop, particularly Architecture 4's predator-scenario context.

#### 13.1.4 Cluster — Tabnine enterprise-governance posture

**Source IDs:** S17 in `sources.md` is the umbrella docs landing; the "Weak or missing citations" section identifies five better-targeted subpages — deployment options, privacy, context engine, agent guidelines, and provenance/attribution.

**Applicability rationale.** Tabnine occupies a different niche than the other three commercial substrates: enterprise-control-first, private-deployment-capable, with explicit governance/provenance/context-control layers. This is the governance-posture angle the §11.10 Round-3 thread was looking for, expressed as a product instead of as a literature review. Pair-dispatch with §11.10 (governance/liability angle) and with §12.3 (El Kaim Council & delegation).

**Target report filename:** `research/21-tabnine-enterprise.md`

**Expected length:** 1500–2200 words.

**Suggested subagent brief.**
> Read the five specific Tabnine subpages flagged in `sources.md` (deployment, privacy, context engine, guidelines, provenance). Compare against §11.10 (Round-3 governance thread), §12.3 (El Kaim L1-L4 delegation in Cluster C), and `architectures/03-phase-gated-foundry.md` (the most governance-aware of our four architectures). Extract: (a) private deployment / VPC / on-prem options and what each constrains; (b) the context engine — how context scoping is governed at the enterprise level; (c) agent guidelines as a typed-instruction primitive; (d) the provenance/attribution model and how it differs from SLSA's (cf. report 12 §1, if it covers SLSA); (e) the privacy posture and what it implies about the lethal-trifecta defense. Output `research/21-tabnine-enterprise.md`. Final section: how the Tabnine governance posture would compose with each of our four architectures.

#### 13.1.5 Cluster — Academic foundations: AlphaCode, SWE-bench, SWE-agent, CodeGen

**Source IDs:** S4 (AlphaCode), S5 (SWE-bench), S6 (SWE-agent paper), S7 (SWE-agent project docs, with a "numbers move" caveat). The "Weak or missing citations" section also flags CodeGen as uncited in the original report.

**Applicability rationale.** Our corpus is weighted toward practitioner reports and product docs. The academic foundations underneath agent coding — large-scale candidate generation and filtering (AlphaCode), multi-turn synthesis (CodeGen), realistic repo-level benchmarks (SWE-bench), agent-computer-interface design (SWE-agent) — are referenced indirectly by Simon Willison and the StrongDM material but never given their own report. The §11.7 Round-3 evals thread would benefit from this calibration; consider pair-dispatching.

**Target report filename:** `research/22-academic-foundations.md`

**Expected length:** 2000–3000 words.

**Suggested subagent brief.**
> Read S4 (AlphaCode), S5 (SWE-bench), S6 (SWE-agent paper), S7 (SWE-agent project docs — note the time-sensitive benchmark numbers caveat in `sources.md`). Find and read a primary CodeGen reference. Compare against `research/05-simon-willison.md`, `research/11-openhands-substrate-audit.md`, and the §11.7 evals thread. Extract: (a) AlphaCode's sampling-filtering-clustering pipeline as an early "orchestration and verification" story; (b) CodeGen's multi-turn synthesis framing; (c) SWE-bench as a realistic-issue benchmark — what it measures and what it doesn't (notably: spec refinement, deployment safety, organizational governance); (d) SWE-agent's agent-computer-interface insight and the 12.5% → 65% → 74%+ benchmark trajectory; (e) where each contributes to the foundations of our four architectures. Output `research/22-academic-foundations.md`. Final section: "What the benchmarks measure that our architectures need" and "what they don't measure that our architectures cover."

#### 13.1.6 Cluster — Anthropic engineering blog trilogy

**Source IDs:** S12 (long-running harnesses), S13 (Agent Skills), S14 (C-compiler with parallel Claudes). The "Weak or missing citations" section also identifies a later application-development harness article that should be cited directly.

**Applicability rationale.** Three official Anthropic engineering posts are cited indirectly across our reports but never collated as a coherent body of primary-source evidence. The trilogy together describes Anthropic's own factory-pattern thinking: long-running harness design (initializer + coding agent + structured handoff), Agent Skills as modular procedural knowledge with progressive disclosure, and a concrete 16-agent / ~2000-session / ~$20k / 100k-line C compiler case study. Pair-dispatch with §11.11 (Compound Knowledge plugin, since Anthropic's Agent Skills is the substrate that the Compound Knowledge plugin builds on) and with §12.4 (El Kaim Codex Cluster D, since both treat skills-as-distributed-knowledge).

**Target report filename:** `research/23-anthropic-engineering-trilogy.md`

**Expected length:** 1800–2500 words.

**Suggested subagent brief.**
> Read S12, S13, S14, and the later application-development harness article flagged in `sources.md`. Compare against `research/04-every-skill-libraries.md`, the §11.11 Compound Knowledge plugin thread, the §12.4 El Kaim Codex Cluster D, and `architectures/02-compound-atelier.md`. Extract: (a) the long-running-harness design pattern (initializer + coding agent + structured update for next session) and how it bridges context windows; (b) Agent Skills as modular procedural knowledge with progressive disclosure — what's loaded eagerly vs. lazily; (c) the C-compiler case study's numbers verbatim (16 agents, ~2000 sessions, ~$20k cost, 100k LOC, can build Linux 6.9); (d) the security considerations Anthropic flags around skills; (e) the application-development harness as the operational analog to the C-compiler experiment. Output `research/23-anthropic-engineering-trilogy.md`. Final section: how Anthropic's Agent Skills compares to El Kaim's Codex (§12.4 Cluster D) and to the Every Compound Knowledge plugin (§11.11) — three independent instantiations of "typed distributed knowledge for AI agents."

### 13.2 Weak-citations QC checklist (for our own future synthesis)

`sources.md` includes a "Weak or missing citations" section enumerating where the deep-research report cited umbrella docs that should have been more specific subpages. This is a **process-level lesson** for our own cross-round synthesis. When writing the still-pending report 13 of Round 2 (`research/13-round-2-synthesis.md` per §3.6) or any later synthesis pass, consult this checklist to avoid the same compression traps:

| Trap | What goes wrong | Mitigation |
|---|---|---|
| Citing the cloud-agent overview for security-scanning claims | Overview page doesn't substantiate Autofix / CodeQL specifics | Cite the dedicated code-review and Autofix/CodeQL pages at the sentence where the detail appears |
| Citing the Codex main page for AGENTS.md / Subagents / approvals / SDK / App Server claims | Single umbrella page can't substantiate five distinct subsystems | Cite each subsystem's official page at the sentence where the detail appears |
| Citing the long-running-harnesses Anthropic post for Skills + later-harnesses content | Three separate Anthropic engineering posts collapsed into one citation | Cite each engineering post at the sentence its content is being used |
| Citing the old Ghostwriter blog for present-day Replit Agent claims | Older product framing can't substantiate current-product features (app generation, `replit.md`, integrations, skills) | Cite the current Replit Agent docs, not the historical product blog |
| Citing the Tabnine docs landing for deployment / privacy / context / guidelines / provenance claims | Umbrella docs page can't substantiate any of the specific governance subsystems | Cite each subsystem's specific docs subpage at the sentence its content is being used |
| Time-sensitive benchmark numbers stated without a date stamp | E.g., the SWE-bench Verified figure has moved from 65% to >74% since the deep-research report was written | Always stamp the date a benchmark figure was captured and cite the specific revision of the docs |

Synthesis-time process: when stating a claim and reaching for a citation, ask "is the page I'm about to cite the *narrowest* official source that substantiates *this specific sentence*?" If no, find the narrower page first.

### 13.3 Counterfactual comparison — at synthesis time

When we eventually write our own cross-round synthesis (report 13 of Round 2, or any later cross-round synthesis), include a deliberate section titled something like **"External-synthesis comparison: what we caught that ChatGPT didn't, and vice versa."** The comparison should answer:

- **Topics the external synthesis covered that our reports don't** — primarily the six clusters in §13.1 (Codex / Copilot cloud / Replit / Tabnine / academic foundations / Anthropic engineering trilogy). After Round 5 dispatches, this gap should close.
- **Topics our reports cover that the external synthesis doesn't** — the El Kaim Dark Factory and book corpus (reports 07 + §12 Round 4), the Jaymin West book (report 09 partial + Round 2's pending 08/10/13), the StrongDM Attractor primary-source detail (report 02), the Round-3 follow-up threads (§11), the Round-4 EA-book clusters (§12). The external synthesis was generated from only the two seed files and therefore could not have surfaced these.
- **Where the two reference architectures differ** — the deep-research report proposes a layered reference architecture (Specification ingestion / Planning / Execution runtime / Testing & evaluation / Orchestration & synthesis / Security & safety controls / Modularization & memory / Dependency & data plane / CI/CD & release control / Runtime monitoring & observability / Governance & legal & IP & reproducibility / Cost & latency optimization / Human-in-the-loop & developer UX). Our `architectures/00-comparison.md` four architectures plus §4.1's shared-infrastructure list cover the same ground in a different shape. The synthesis pass should diff the two structurings, identify any cell our four-architecture frame misses, and decide whether our frame should be extended or whether the external frame has elements we deliberately don't want.
- **Where the gap analysis identifies needs our four architectures already address** — useful as a *validation* of our existing architecture work; if a gap the external synthesis flags as "currently unmet" is already in our Architecture 2 or 3, that's a strong signal we're on the right track. Conversely, gaps it flags that *aren't* in any of our four architectures are candidates for the still-pending architectural revisions.
- **Methodology comparison** — what one-shot deep-research caught vs. what multi-round skill-driven research caught. This is the methodological output of the comparison: useful for choosing how to scope future research projects.

This comparison is **not** a Round-5 dispatch — it's a deliberate section to add when the synthesis pass is written. Record it here so the synthesis author doesn't forget.

### 13.4 Notes for subagents (carry over §12.7 conventions)

- Reports land in `research/18-…md` through `research/23-…md`, continuing the main numbering after Round 4's 14–17 reservations in §12.
- Cite each source by its `sources.md` S-ID (e.g. "S9 — OpenAI harness-engineering blog") in addition to the URL, so cross-references to `research/external-syntheses/chatgpt-deep-research-2026-05-11/sources.md` are traceable.
- Apply the §13.2 weak-citations checklist while writing — narrowest official source per sentence.
- Time-stamp every benchmark number (e.g. "SWE-bench Verified, mini-SWE-agent: 65% as of 2026-05-10 per S7, advanced to >74% per current project docs").
- The six clusters in §13.1 are independent and parallelizable; can be dispatched as one wave of 6 subagents, or paired strategically:
  - 13.1.2 (Copilot cloud) + §11.6 (competitor landscape)
  - 13.1.4 (Tabnine governance) + §11.10 (governance/liability) + §12.3 (El Kaim Council)
  - 13.1.5 (academic foundations) + §11.7 (evals deep-dive)
  - 13.1.6 (Anthropic engineering trilogy) + §11.11 (Compound Knowledge plugin) + §12.4 (El Kaim Codex Cluster D)
- Do NOT modify `research/external-syntheses/chatgpt-deep-research-2026-05-11/*` — those files are preserved verbatim. Updates flow through the new reports and through any future README revision.
