...
- **Lock-in / undefined-term debt (inventory one-liner)** — Mitigation (DELTA-06): every Gas City term carries an `extraction_safe_synonym` and a `lock_in_cost` note, so the vocabulary is *recoverable* (the runtime substrate stays swappable) rather than a one-way trap. Aligns with AI-CONTEXT §3.6's "tmux runtime is deliberately vocabulary-free" finding. **Named consumers (so these are not dead metadata):** `extraction_safe_synonym` is the term-level migration map for the **C01-B DELTA-01** portability contract (if Gas City is re-platformed, this is how each term is renamed); `lock_in_cost` feeds the **C57** residual-risk register's vocabulary-lock-in row. If neither consumer is ratified, DELTA-06 should be dropped (integrator call).
