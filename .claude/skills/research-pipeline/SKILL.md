...
| Propose, name, or register a new failure mode (F-mode) | [`resources/_drain/stage-5-content-processing.md`](resources/_drain/stage-5-content-processing.md) "Failure-mode discovery and registration" + [`architecture-failure-mode-gate`](../architecture-failure-mode-gate/SKILL.md) skill (owns the table + CI gate) |
| Edit the failure-mode coverage table or any `architectures/0N-*.md` | [`architecture-failure-mode-gate`](../architecture-failure-mode-gate/SKILL.md) skill |
...
10. **[`architectures/failure-modes.md`](../../../architectures/failure-modes.md) is the canonical index of failure modes.** Whenever a report proposes a new failure mode, register it in [`architectures/failure-modes.md`](../../../architectures/failure-modes.md) in the same commit. Renumber on collision; propagate the renumber to every reference (grep with word-boundary `\b`). Discovery procedure (this skill): [`resources/_drain/stage-5-content-processing.md`](resources/_drain/stage-5-content-processing.md) "Failure-mode discovery and registration". Table edits, schema, and CI gate are owned by the separate [`architecture-failure-mode-gate`](../architecture-failure-mode-gate/SKILL.md) skill — load it whenever you touch the file directly.
