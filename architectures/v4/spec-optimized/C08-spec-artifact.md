...
`spec.md` MUST contain four labelled sections: **Goal**, **Constraints** (functional + non-functional, per ArchCode/PRDBench, Part 2), **Definition-of-Done** (pointer to `DoD.md`), **Out-of-scope**. This is the minimum that lets C10 lint *vocabulary + completeness* (not just prose form) and gives the under-specification failure F3 a structural surface. **Enforcement is graded by `detail_level` (composes with DELTA-06):** `vague` requires only **Goal** (the rest are advisory-until-clarified — the clarification hook fills them before build tokens are spent); `moderate`/`complete` require all four. This lets a deliberately thin spec be committed and safely driven to completeness rather than rejected up-front.
...
- **INV-2 (well-formed bundle, graded).** The manifest resolves; `DoD.md` parses as an enumerated checkable list; and the required sections exist **per `detail_level`** (`vague` ⇒ Goal; `moderate`/`complete` ⇒ all four — DELTA-05/06). A bundle failing this is rejected before any run (replaces the faithful "must parse as a Go template" — renderability is now C09's concern, not C08's, **pending the C08↔C09 seam-ownership ruling, OQ1/DEFERRED**, DELTA-01).
...
|---|---|---|
| **F18** Prose specs lack rigor | Free-form prose is ambiguous. | Required-section schema (DELTA-05) + enumerated, per-criterion-scoreable **DoD** (DELTA-03) move the spec from diffuse prose to an *addressable* contract; C10 lints the sections. **Honest scope:** the F18 rigor gain is real *to the extent a DoD criterion is deterministically or scenario-checkable* (test-pass, lint, scenario); a *judge-only* criterion (C32) inherits residual prose ambiguity — but now **localized and labelled per criterion** rather than diffuse across the whole spec, and still scoreable for C33 partial credit. Stronger than faithful "Partial", but not "fully deterministic". Requires the criterion taxonomy {deterministic \| scenario-backed \| judge-only} (AC + OQ2, sweep-2). |
...
4. **AC-4 (DoD is enumerated + classified, DELTA-03).** Each `DoD.md` criterion has a stable id, a boolean/scored evaluation, and a declared **check class** {deterministic \| scenario-backed \| judge-only}; C32/C33 can compute "fraction satisfied" and report which fraction is deterministically vs. judge-evaluated. *A sample DoD scores deterministically on its deterministic criteria and routes judge-only criteria to C32.*
...
- **OQ3 (→ review-log).** *`spec_id` granularity (DELTA-04).* Largely resolved by the **lineage+revision** split (RC08B-01): satisfaction history keys on `spec_lineage_id` (continuous across edits) and annotates each point with the `spec_id` measured — so a DoD-only edit is a visible step in one series, not a severed history, without needing per-file addressing. Per-file content-addressing remains a *further* optimization (deterministic attribution of *which* file changed) — deferred to sweep 2 with C21.
