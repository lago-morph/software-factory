...
- **WS-B (surfacing + conversion):** T4 + T5 + T6 — built against a **seeded `override` log fixture** (no need to wait for WS-A to produce real beads). The SQL/DuckDB surfacing (T4) and the rule-sink handoff(s) (T6) are independent sub-streams. The v4-named **C30** rubric handoff is the must-build; the inferred **C10 / C15** handoffs (if retained — spec OQ4) are additional independent sub-streams that do not block each other.
...
4. **Rule-emission handoff per sink** (T6) — the target rule schema(s): **C30 (Inspect-AI rubric)** is the v4-named sink and the one to freeze first; C10 (EARS/INCOSE rule) and C15 (Mammoth rule) are the *inferred* sinks (spec §3 contract-6 [FAITHFUL-FILL]) to freeze only if the orchestrator retains them (spec OQ4). Coordinate each with the sink owner; these are *their* schemas, C35 only targets them.
...
2. **Multi-sink conversion fan-out (T6).** Rule conversion targets the v4-named **C30** rubric schema and, if retained, the *inferred* C10/C15 schemas (spec OQ4) — potentially three different rule schemas, the riskiest integration surface. **First resolve scope** (Inspect-AI-rubric-only vs +C10/C15) with the orchestrator, then de-risk by contract-testing each retained handoff against a sink stub early; do not assume one unified rule format.
...
- **Per spec ACs:** AC1 (override → hook → predicate → one `override` bead with `created_by`), AC2 (non-empty "why", or `why_missing`, never dropped), AC3 (surfacing groups a recurring class, reports none when sparse), AC4 (approved pattern → new rule in the right sink — the v4-named C30 Inspect-AI rubric, or the inferred C10/C15 sink if retained — reachable only via the operator gate), AC5 (writes only C20-declared fields), AC6 (native hooks only, no custom hook engine), AC7 (no override exists only in chat — all retrievable by type).
...
- **Component DoD:** a real operator override flows detect → why → log → (after seeded recurrence) surface → operator-approve → a proposed rule registered in the v4-named C30 rubric sink (and the inferred C10/C15 sinks if retained — spec OQ4); and the G43 maturity reconciliation (automated scope = detect→surface, conversion = human-gated; F10 valid from Phase 3a) is written to `_meta/review-log.md` — closed by escalation, not silent assumption.
