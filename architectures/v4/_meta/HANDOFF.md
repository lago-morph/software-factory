...
## 0★. TOP PRIORITY: implement the spec–scenarios–system triangle (ADR-0069 / D-42)

> **✅ §0★ DONE — triangle implemented into the eval-tier specs 2026-06-02** (stacked PR off `main`, ready-for-review). C32 reframed scorer → **diagnostician** (adds `diagnose()` + the frozen companion **`DiagnosisRecord`**, §3.2a — `ScoreRecord` D-39 freeze preserved); C53 completion = **tri-alignment** (four conjunctive go terms; 100% floor pinned, distribution → evidence); C52 fail-branch → **repair router** keyed on the judge's attribution (anti-gaming: spec/scenario correction never the worker); C30 states the **S↔H correspondence duty** + the in-system-tests-vs-hold-out distinction; C08 states the **spec-quality properties** + the independent spec-correction path; C34 confirms/extends the **H↔I anti-gaming independence** + audits the `DiagnosisRecord` + the correction-request provenance. New ledger: **D-43** (DiagnosisRecord contract), **D-44** (canonical correction-request payload), **D-45** (C34 = canonical worker-origin-rejection enforcer; resolves C08:OQ-3). `auto-002` reframed (its gate is the H↔I edge within tri-alignment). Verified by a per-product opus seam adversary (frozen records clean — no run-#2-class drift) + a cross-product integrator pass. **CARRIED-FORWARD follow-ups (out of the §0★ six-spec scope):** register `softwarefactory.v4.beads:diagnosis_record` in **C22**; add `diagnosis_bead_ref` to the **C20** `factory_build` bead — both additive/non-breaking, tracked as ledger OQs under D-44/D-45. The text below is the original brief (kept for context).
>
> Canonical statement: [**ADR-0069**](../../../docs/adr/0069-spec-scenarios-system-triangle-evaluation-invariant.md). Ledger: **D-42** ([review-log.md](./review-log.md)). This was the **#1 next task**, ahead of "Sweep-2 for the remaining 32 components." The eval tier as Sweep-2-specced modelled the judge as a *scorer*; the operator established that the judge must be a *diagnostician* over a three-representation triangle. **Read the intent below before touching any spec — the mechanics only make sense in light of it.**
