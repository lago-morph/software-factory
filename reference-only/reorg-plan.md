...
Six interim **subject-matter** categories. (A first pass tried medium-based categories — `books`, `essays`, `vendor-docs`, `academic-papers`, `podcast-transcripts`, `external-syntheses` — and was rejected by the orchestrator in favour of subject-matter; see Lessons learned below.) Sizing is currently small; the eventual restoration step will refill these, and a future iteration of categorization may refine — likely by splitting `practitioner-harnesses` and `substrate-and-skills` once they cross the ~15-source threshold.

| Short-name | Definition | Current sources | Notes (cross-cutting / alternatives considered) |
|---|---|---|---|
| `dark-factory-methodology` | The intent-driven / dark-factory paradigm: how software gets specified, automated, and built when AI is doing most of the work. El Kaim, Shapiro, and the surrounding methodological tradition. | `el-kaim-book` (9 chapters), `dark-factory-article` | — |
| `practitioner-harnesses` | Personal harnesses, compound-engineering workflows, and lived-experience accounts of practitioners using AI in their daily build loop. | `every-my-ai-had-already-fixed`, `lenny-podcast-transcripts` | `lenny-podcast-transcripts` is cross-cutting: the Willison segment (dominant by word-count) is practitioner state-of-the-union; the Cherny segment is really `substrate-and-skills`. Picked `practitioner-harnesses` because Willison's full transcript is the larger half. |
| `substrate-and-skills` | Vendor implementations of AI development substrate + composition primitives (skills, tools, MCP, sandbox semantics). | `anthropic-agent-skills` (docs pages + 3 cookbook notebooks) | — |
| `security-primitives` | Threat models, prompt-injection defenses, capability/data-flow security designs for agentic systems. | `camel-paper` (LaTeX source) | — |
| `ai-engineering-culture` | Team-level dynamics, organizational culture, and the social/operational side of AI-engineering practice (as distinct from the personal-workflow level). | `brier-culture-of-ai-engineering` | — |
| `meta-synthesis` | Derived syntheses over the corpus (counterfactuals from other AI systems, deep-research one-shots, QC-oriented re-reads). Distinct from primary sources because they are second-order. | `chatgpt-deep-research-2026-05-11` | Considered classifying as `substrate-and-skills` (it is a ChatGPT product output) and rejected — its role here is counterfactual synthesis, not substrate documentation. |
