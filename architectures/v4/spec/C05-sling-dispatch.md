...
| **Pool-exhaustion** (interface-local) | A pool exists but all members are busy/unavailable. | Faithful: handled outside C05 — Gas City native dispatch may apply back-pressure (DELTA-02 dropped as native), and/or no available member ⇒ treat as no-target *for this tick* (INV-3) so the reconciler retries next tick (C18). C05 holds no queue (see §7, OQ-3). |
...
7. **AC-7 (attributable dispatch, native).** Each routing decision is observable as "W → agent/role A under template T" via the **native** `created_by` (C41) + event-bus append (C23) that fire on the dispatch — verified to be present, *not* produced by a C05-owned record (INV-4, §3.2).
