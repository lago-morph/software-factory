...
**Methodology-survey note (added 2026-05-16, Cluster N drain).** Three companies surfaced in the Cluster N drain — **Tenex** (CJ Hess's employer; alternative-affiliation framing "10X" flagged in report 34 header), **Notion** (Ryan Nystrom's employer; report 35), and **Sendbird** (John Kim's company; report 36) — are *not competitors* in the AI-coding-agent / software-factory product space. They are **users / methodology benchmarks**: companies whose internal AI-adoption practices are corpus-relevant for the *operator-side* of the methodology, not as vendors of AI-coding agents. Brief positioning:

- **Tenex** — small SWE shop; CJ Hess's personal-harness ecosystem (Flowy + `kevin`/`carl` model-vs-model QC) is a *practitioner methodology* anchor, not a product. Worth a follow-up fetch if Tenex publishes any team methodology.
- **Notion** — productivity-app vendor; Nystrom's *"Boxy"* internal background-agent system + *"Project Afterburner"* CI-speedup project are corpus-anchor *practices*. **Naming-collision flag**: Notion uses *"software factory"* as an internal-marketing alias for Boxy; this collides namespace-wise with **8090's "Software Factory" product line** (§2 above) and with the corpus' own *software-factory* abstraction. When "Software Factory" appears in corpus discussions going forward, disambiguate: (a) 8090's product line; (b) Notion's Boxy internal-marketing alias; (c) the corpus' own concept. Use "Boxy" for the Notion system.
- **Sendbird** — communication-API vendor; Kim's *"Automators"* quest marketplace + per-employee token-tier leaderboard ("AI God" tier) are corpus-anchor *org-design* practices. Internal AI-product surface "Delight.ai" naming flagged for followup.

Together with **Stripe** (referenced by both Cherny in followup/03 and Nystrom in report 35 — *"1,300 agent-PRs/week"*) and **Glowforge** (Shapiro's company, report 32 — "claw-printer / one-Claw-per-employee"), these comprise the **operator-methodology benchmark set** for the corpus. They are not vendors; they are practice anchors.
...
**Note added 2026-05-16 (Cluster F manual drain).** None of the five named competitors above publishes per-engineer agent-driven throughput numbers comparable to a 1P operator benchmark. The closest such anchor in the corpus is OpenAI's own internal-product experiment, drained from primary on 2026-05-16: Ryan Lopopolo, *"Harness engineering: leveraging Codex in an agent-first world"* (OpenAI, 2026-02-11; `openai.com/index/harness-engineering/`). The numbers — three engineers ramping to seven; on the order of **1 million LOC** across application logic, infrastructure, tooling, docs, and tests; **~1,500 merged PRs** in five months; **3.5 PRs/engineer/day as the small-team baseline with per-engineer throughput *increasing* as the team grew**; an estimated **~1/10th the time** of the human-coded counterfactual; and routine **six-hour single-agent runs** — set a quantitative reference point that the named competitors here either do not publish or do not match in granularity. See [`18-openai-codex-substrate`](../18-openai-codex-substrate.md) §5 (primary-anchored 2026-05-16) for the full quotation set and the canonical caveats (no defect-rate counterfactual; team self-estimated baseline; ongoing experiment, not terminal state). When the named competitors publish comparable per-engineer numbers — Devin's "Managed Devins," Factory's role-scoped droids, Superconductor's multi-agent-on-the-same-ticket — they should be read against this benchmark, not against a human-coded baseline. The "compounding teams" framing (per Schillace, `research/manual/I have seen the compounding teams - by Sam Schillace.txt`) is the conceptual umbrella; Lopopolo's piece is the first quantitative attestation from inside a large vendor.

---

## Microsoft Amplifier — now anchored in report 28

**Note added 2026-05-16 (Cluster I manual drain).** Amplifier was previously flagged in this report (§ on Superpowers and Open follow-ups) as a Schillace/Krabach-authored framework "worth a competitor-landscape-style write-up" — surfaced obliquely by Jesse Vincent as a sibling self-improving-via-SKILL.md framework. With the Cluster I drain, Amplifier internals are now **primary-source anchored** in [`28-schillace-sunday-letters`](../28-schillace-sunday-letters.md) and do not need re-draining here. The full picture (per report 28 §4 and §5): a yaml-based **recipes** workflow primitive (recipes can compose into "foundries" — e.g., Schillace's book-writing foundry); a **Context Query** XML-categorised question-answering pattern explicitly credited with hallucination reduction; **subagents and containers** for task hand-off and environment isolation; an **external-memory orchestration** layer that compensates for the model's lack of new-memory formation; and a **self-improving "gene transfer" loop** built on three named expert subagents — a **session analyst** (reads past harness sessions for pattern extraction), a **foundation expert** (understands the system's own architecture), and a **"Crusty Old Engineer" critic** (adversarial design-feedback role; the equivalent of OpenAI's `Auto-review` subagent at report 18 §4.4 or Arch 4's "predator agent" at report 09 §5.5). Schillace lineages Amplifier back to **Microsoft Semantic Kernel** (named "on a whim, as a code name" in late 2022) as the proto-orchestrator/proto-harness — a lineage claim that is corpus-novel and that should fold into the harness-history chart in report 09. The **Dev Foundry** primitive — *"a 'dev foundry' that I built that stamps out bespoke 'dev machines' that can work on tasks for days or weeks on their own"* — is semantically a sibling of Devin's "Managed Devins" and Factory's "Droid Computers" but framed factory-side (stamping out) rather than agent-side. Amplifier is open source on GitHub (`github.com/microsoft/amplifier`); the in-Schillace team's operating context is *"a generous token budget"* from Microsoft (no public dollar figure). Cross-link to report 28 for the eleven-source primary anchoring; this report no longer needs its own Amplifier write-up.
...
- **Microsoft Amplifier** (`github.com/microsoft/amplifier`) — ✅ **resolved 2026-05-16 (Cluster I drain)**: now primary-anchored in [`28-schillace-sunday-letters`](../28-schillace-sunday-letters.md). See the "Microsoft Amplifier — now anchored in report 28" section above. Residual open question: read the GitHub repo's SKILL.md / yaml-recipe files directly for the exact schema of a "recipe," and confirm/refute the "session analyst" + "foundation expert" + "Crusty Old Engineer" agent definitions are stored as version-controlled markdown+yaml files in the same way Vincent's Superpowers stores SKILL.md.
...
| `sundaylettersfromsam.substack.com` (Schillace on Amplifier) | ✅ primary (Cluster I drain, 2026-05-16) | All 11 Sunday Letters drained into [`28-schillace-sunday-letters`](../28-schillace-sunday-letters.md); Amplifier internals (Dev Foundry, recipes, Context Query, session analyst, foundation expert, Crusty Old Engineer, gene transfer) primary-anchored. |
