...
- **Phase-4-internal workstreams (Wave 4.5 ownership).** Four BF-L workstreams owed at Wave 4.5:
  1. **Conventional-view smoke-test subagent** — produces `sub-tracks/bfl-conventional-smoke-test.md` per the [`auto-003` Round 2 smoke-test recipe](../decisions/auto-003-bfl-rg-view-choice.md#phase-35-follow-up-smoke-test-new-wave-45-pre-lead-agent-coordinated). Consumes [`bfl-conventional-view-prior-art.md`](../research-notes/bfl-conventional-view-prior-art.md) as prior-art frame.
  2. **Invariant-view smoke-test subagent** — produces `sub-tracks/bfl-invariant-smoke-test.md`. Consumes [`bfl-invariant-view-prior-art.md`](../research-notes/bfl-invariant-view-prior-art.md) as prior-art frame.
  3. **Conditional Wave 4.5b full sub-track** — if either smoke-test verdict authorizes (≥2/3 languages), 1-2 additional subagents scale the recipe from 3-per-language to ≥10-per-language for the qualifying view(s). Wave 4.6 renders the go/no-go.
  4. **Methodology-degradation clause specification at Phase 6** (seeded from Phase 4) — convention fallback → no-convention regions default to L3 pending operator approval; invariant fallback → derived scenarios flagged "invariant-unconditioned" and routed to cross-family review (per [`auto-003` Round 2 clause](../decisions/auto-003-bfl-rg-view-choice.md#methodology-degradation-clause-new-per-reviewer-2-a2)).

- **Phase-5 ADR seeds.** (i) P-26 integrated-view schema (ID space, join API, snapshot-consistency / Merkle-DAG versioning); (ii) P-27 scenario-derivation / archaeological-brief calibration (hold-out vs cross-family-diff vs adversarial reviewer); (iii) P-19 BF-L variant feature-source design (feature schema, OPA hard floors, conditional-feature handling, classifier drift monitoring); (iv) two per-view ADRs (conventional / invariant) citing the Wave-4.5 verdict and Wave-4.4 research notes as alternatives-considered, status `accepted` or `accepted-with-RG-flag` per the [Phase-3.5.5 rule](../candidate-registry.md#phase-355-rule-on-load-bearing-rg-primitives-binding-user-approved-2026-05-25).

- **Phase-8 lean-eval candidates.** (i) P-26 integration discipline at 1M+LOC / 10+ year history (soft RG on scale); (ii) per-region regime classifier drift under degradation regime; (iii) maintenance-loop cadence calibration (own OQ-T3); (iv) Codebase Model F54 attack-surface pressure-test.

- **F-mode carries.** F25, F40, F47 remain accepted-open or partially-addressed per BF-L track §2; no new substrate-layer carry surfaces here.
...
- No primitive pre-eliminated; all 5 primitive families carry forward, including the load-bearing P-26.
- The two RG views are **honestly surfaced** — the (a)/(b) choice is explicitly the auto-003 Round 2 smoke-test pattern, with the (b) fallback operationalized via the methodology-degradation clause (not a fig leaf).
- The candidate's load-bearing claim (three loops over one Codebase Model; methodology parameterized by ingestion fidelity) is preserved; the conditional path means BF-L survives in either branch of the Wave-4.5 verdict.
- No same-vs-distinct verdicts rendered for BF-L's P-19 variant vs other candidates' variants — deferred to Phase 4.2 per the [Round-1 amendment to `auto-001`](../decisions/auto-001-phase-3.5-dispatch-shape.md#amendments-to-the-dispatch-brief-that-will-be-sent-to-subagents).
- BF-S / BF-M asymmetry justified on **load-bearing magnitude** per [`auto-003` Round 2 Reviewer 2 A4](../decisions/auto-003-bfl-rg-view-choice.md#bf-sbf-m-asymmetry-justification-new-per-reviewer-2-a4): P-26 is BF-L's entire thesis; BF-S's B7 is a measured residual; BF-M's P-27 calibration is one input among many.

BF-L survives Phase-4.1 with the load-bearing RG flag carried honestly into Wave 4.5; Phase-3.5.5 status (`survives with research-grade-uncertainty flag on Codebase Model`) is confirmed. Scoping principle preserved: no shrinkage forced; the conditional path is real on both branches; the honest framing of P-26 as the most ambitious primitive in the catalog is maintained.
