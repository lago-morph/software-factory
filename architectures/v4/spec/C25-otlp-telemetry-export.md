...
- **OQ-1 (→ [review-log](../_meta/review-log.md), top open question) — G04 two-sinks statement.** v4 is consistent but never says "two separate sinks" out loud, so a naive integrator may wire OTLP → CXDB (the rejected path: CXDB has no OTLP receiver, AI-CONTEXT:210 / §5.2; rejection at AI-CONTEXT §11.1:466 + §11.3:497). **Faithful resolution adopted:** INV-1 + AC-6 state explicitly that the OTLP channel terminates at C26 only and CXDB is fed *only* by raw-bodies → C24. Open item for the reviewer: should this two-sinks rule be hoisted into a shared Observability-subsystem note (C25/C26/C24) so all three specs assert the same boundary, rather than living in C25 alone? (Reading A: state once in C25 as the source; Reading B: restate in each. Faithful pick: state in C25 — the source — and cross-reference from C24/C26, since C25 is where the split originates.)
