...
## Drain note (Cluster F manual MHTML capture) — 2026-05-16

The OpenAI *"Introducing SWE-bench Verified"* blog has been **flipped ❌ → ✅** via manual MHTML capture drained on 2026-05-16 (`research/manual/Introducing SWE-bench Verified _ OpenAI.txt`). The canonical text adds material that the swebench.com/verified proxy did not surface, primarily:

- **Preparedness Framework framing** (verbatim): *"As part of our Preparedness Framework, OpenAI develops a range of metrics to track, evaluate, and forecast models' abilities to act autonomously. The ability to autonomously complete software engineering tasks is a key component of our **Medium risk level in the Model Autonomy risk category**."*
- **Three under-specification / unsolvability categories** the team identified before launching annotation: overly-specific unit tests; underspecified issue descriptions; un-reliable dev-environment setups (all verbatim, see §4 below).
- **Annotation methodology**: 93 Python-experienced software developers, 1,699 random samples annotated, 4-level severity scale ([0,1,2,3]), ensemble-of-3-annotators with highest-severity-wins, onboarding tests gated by 50 hand-labeled samples.
- **Filter mechanics**: filter out any sample where problem-statement OR FAIL_TO_PASS-test severity ≥ 2 OR "other major issues" flag — leading to **68.3% of samples filtered out** of the 1,699-sample subsample; arrival at the 500 by including all 1–4 hour and >4 hour difficulty samples plus a random selection of remaining ≤1 hour samples.
- **Re-measured GPT-4o lift**: 16% on original SWE-bench → 33.2% on SWE-bench Verified, *"more than doubling its score"*. The blog explicitly states this *"validates our initial suspicion that the original SWE-bench dataset underestimates agent abilities."*
- **Difficulty-stratified performance**: performance increases *within each difficulty category* moving from Full to Verified, confirming the improvement is from removing impossible samples, not from re-weighting toward easy ones.
- **The 'easy' / 'hard' annotation slice**: 196 samples are <15-min fixes; 45 samples are >1-hour. Released alongside the 500-instance Verified subset for difficulty stratification.
...
**Status caveat on CodeGen citation.** The Round-5 sources audit (`reference-only/chatgpt-deep-research-2026-05-11/sources.md` §"Weak or missing citations") flagged CodeGen as one of the named-but-uncited references in the original report. This report adopts arXiv:2203.13474v5 as the canonical primary citation; the OpenReview record at https://openreview.net/forum?id=iaYcJKpY2B_ confirms the ICLR 2023 venue and review status.
...
- **SWE-bench Verified** — per swebench.com/verified: "**A human-validated subset of 500 SWE-bench instances** for reliable evaluation of coding agents and language models. … Human annotators reviewed each instance to ensure the problem descriptions are clear, the test patches are correct, and the tasks are solvable given the available information." Released in collaboration with OpenAI in August 2024 ("[08/2024] SWE-bench x OpenAI = SWE-bench Verified" per swebench.com home news entry). **[2026-05-16 SWE-bench-Verified primary ✅]** The OpenAI blog announcement adds the Preparedness-Framework framing (verbatim): *"As part of our Preparedness Framework, OpenAI develops a range of metrics to track, evaluate, and forecast models' abilities to act autonomously. The ability to autonomously complete software engineering tasks is a key component of our Medium risk level in the Model Autonomy risk category."* The blog also names three concrete failure modes the team identified in the original SWE-bench (verbatim): (1) *"The unit tests used to evaluate the correctness of a solution are often overly specific, and in some cases are even unrelated to the issue"*; (2) *"Many samples have an issue description that is underspecified, leading to ambiguity on what the problem is and how it should be solved"*; (3) *"It is sometimes difficult to reliably set up the SWE-bench development environments for the agents, inadvertently causing unit tests to fail regardless of the solution."* These three failure modes are the academic-side mirror of every "spec → code gap" finding in report 26 — the under-specification problem is not a hypothesis; it is the empirically-measured load-bearing fault in pre-Verified SWE-bench.
...
- *Linear history is load-bearing.* mini-SWE-agent's "completely linear history" is the same property that OpenHands' V1 SDK enforces via event-sourcing ([`11-openhands-substrate-audit`](11-openhands-substrate-audit.md) §4.2) — and that the Anthropic harness blog endorses for long-running agents.
- *Bash + tests is enough for many tasks.* This is the academic vindication of Simon Willison's "First run the tests" + AGENTS.md + `python -c` substrate ([`05-simon-willison`](05-simon-willison.md) §"Agentic manual testing"). The factory does not need a bespoke tool surface for everything.
...
A compact mapping (referenced against [`00-comparison`](../architectures/00-comparison.md) and the spec-driven baseline):
...
5. **Multi-PR / multi-service coordination.** Real software changes cross PRs, services, and teams. SWE-bench is single-PR. The architectures must define how multi-PR work is decomposed, coordinated, and rolled back as a unit. (The Anthropic C-compiler case study — `reference-only/chatgpt-deep-research-2026-05-11/` S14 — is the closest practitioner analogue.)
6. **Cost, latency, and human attention budgets.** SWE-bench reports pass@1 but not tokens or dollars. mini-SWE-agent's "100 lines, >74%" is partly a cost argument the benchmark does not score. The StrongDM "$1,000/day per engineer" floor ([`05-simon-willison`](05-simon-willison.md) §"Wait, $1,000/day per engineer?") is *the* practitioner constraint that academic work elides. Each architecture must answer it.
