...
| Upstream (verification tooling) | **C17** Tool-node abstraction · **C02** Pack ABI | > [FAITHFUL-FILL] — **not a declared inventory edge**; faithful fill. The contract check (Pact/schemathesis) and the behaviour diff over record/replay-captured references run as **Gas City pack tool nodes** ("Gas City pack per service", README:201; "Per-language tool node", README:199), which is C17/C02's surface. C45 packages its checks as tool nodes; it does not redefine the ABI. |
