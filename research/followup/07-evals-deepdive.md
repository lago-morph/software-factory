...
**Plan reference:** [`PLAN`](../PLAN.md) §11.7
...
The new **report 29 — Prompt Engineering Survey** ([`29-prompt-engineering-survey`](../29-prompt-engineering-survey.md), drained 2026-05-16 from Schulhoff et al.'s *The Prompt Report*, arXiv:2406.06608v6) is the **academic taxonomy anchor for this thread's LLM-as-judge discussion**. Report 29 §5 (*LLM-as-judge — the §4.2 evaluation-prompting catalogue*) summarizes Schulhoff et al.'s five-pattern catalogue: **single-output judging**, **pairwise judging**, **reference-based judging**, **self-judging**, and **multi-criterion judging**. Each maps onto a pattern already discussed here:
...
**Implication for our docs:** the project plan in [`PLAN`](../PLAN.md) and the per-architecture roadmaps should count **experiments run per week** as a leading indicator, with feature ship dates as lagging indicators contingent on experiment outcomes. The Foundry's phase-gate cadence and the Tournament's generation cadence are already structured this way; the Refinery and Atelier roadmaps should adopt the same framing.
...
### Cluster-O cross-reference (2026-05-16) — SABM as candidate multi-agent eval methodology

The Cluster-O drain landed [`37-academic-llm-agent-collusion`](../37-academic-llm-agent-collusion.md) (Neves & Bussmann, Stanford Computational Antitrust Vol. 6, 2026) and surfaces a candidate evals-methodology pattern that is operationally distinct from the four currently anchored in this followup (Husain's binary-per-AC + Critique Shadowing, Anthropic multi-agent orchestrator-worker, Shankar's three-gate framework, AttractorBench's tiered-conformance). **Smart Agent-Based Modelling (SABM)** is *generative-eval-via-LLM-agents*: the agents themselves *are* the evaluation surface (their emergent behaviour is what is measured against theoretical benchmarks such as the Bertrand-Nash $6.00 / monopoly $8.00 prices in the paper's calibration), rather than producing a measurement of a separate system under test.

The closest current cousin is the Anthropic multi-agent pattern (§2 of this followup), but inverted — Anthropic's lead-and-subagent fan-out *uses* multiple agents to produce a measurement of an external answer; SABM *measures* the multi-agent system itself. The pattern is corpus-relevant for three reasons:

1. **Reproducibility budget.** Neves & Bussmann document a per-run cost of $0.18 (Gemini 2.0 Flash) to $1.51 (Sabiá-3) for a 400-round / 2-agent simulation. The combinatorial sweep (2 models × 2 languages × 3 conditions) implies ~$10–20 of API spend for a full Bertrand-duopoly experimental matrix — within the factory's per-experiment evals budget without research-grant funding.
2. **Theme-2 alignment-drift eval surface.** The corpus currently anchors Theme-2 drift / collusion as a *concern* (reports 02, 28 §3.4, 30, 31) but lacks an operational eval surface to detect it. SABM in a calibrated game-theoretic setting (Bertrand duopoly with known competitive + cartel benchmarks) provides one: median-price-above-equilibrium, percentage-of-rounds-supra-competitive, rolling volatility, Kolmogorov-Smirnov distributional convergence. The five-criterion convergence battery from the paper's Annex A (percentile band convergence + rolling volatility + coefficient of variation + trend convergence + Kolmogorov-Smirnov distributional convergence) is directly portable to *any* multi-agent factory evaluation, not just market-conduct settings.
3. **Discussion-as-amplification (F49) as an evals-design hazard.** Neves & Bussmann's *"mimicking concerns about collusion"* finding (report 37 §6) — agents discuss the failure mode and produce a softer / deniable version of it — has direct implications for **how to phrase safety prompts in eval test cases**. Putting *"don't do X"* in the eval prompt is not a reliable test of whether the system avoids X. The Schulhoff §5 sycophancy paradox (report 29 §4) and the Neves-Bussmann mimicking effect converge on this: eval-prompt phrasing is itself an experimental variable that must be tested per-model-per-language-per-harness-configuration.

**Action:** flag SABM as a candidate methodology for a future evals-substrate drain. The Neves-Bussmann setup is the most cost-efficient calibrated demonstration of Theme-2 drift available in the corpus and should be considered as a reference point for any multi-agent eval suite the factory ships.

---

## 6. Open follow-ups

- Cross-reference with [`09-jaymin-book-harnesses-practices-mental-models`](../09-jaymin-book-harnesses-practices-mental-models.md) for how Overstory frames its own evaluation — Jaymin's framework may already encode some of Husain's discipline implicitly.
- Map specific judge prompts in [`01-specification-refinery`](../../architectures/01-specification-refinery.md) §judge into the binary-per-AC pattern as a concrete worked example.
