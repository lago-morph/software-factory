...
| **F37** Silent contradictory-prompt collapse | If a template renders internally contradictory instructions, the agent may silently collapse them. | v4's guard is multi-model paraphrase divergence (gene-transfusion from GF-M, runnable as a pack) — **Partial** (F-MODE F37). Not a C09-native guard; C09's contribution is INV-1 determinism so the contradiction is at least *reproducible* and diagnosable. |
...
- **Security.** C09 renders untrusted-by-construction human Markdown into an instruction; the security posture is that the *template author* is trusted (it is version-controlled + attributable, C41) and secrets do not belong in templates (credential handling is C03/`city.toml`, not C09 — G37 scope). Template injection via uncontrolled variables is bounded by the render context being run-internal, not external input, at the faithful Phase-0 scope.
- **Cost / scale.** Rendering a Go `text/template` is negligible compute. The scale concern is dispatch throughput (C05 sling) and the agent-side Max rate-limit ceiling (G34) — both outside C09. C09 adds no material cost.
