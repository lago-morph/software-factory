# C25 — OTLP telemetry export (`otlp-telemetry-export`)  (Build Plan, canonical track)

> Source / Spec ref: [`spec/C25-otlp-telemetry-export.md`](../spec/C25-otlp-telemetry-export.md)
> Canonical track. Sweep 1. Depends on: C28 (Claude Code agent loop), with config carried by C03/C04. Non-foundational `interface` in the Observability subsystem; first stage of the C25 → C26 → C27 pipeline and the C25 → C24 raw-bodies feed. Batch-2 per the [component inventory](../_meta/component-inventory.md) (observability ingest C25/C26/C27/C24).
...
2. **Emitted-signal + correlation contract (T2):** the metric/event/trace sets and correlation attributes, with **`session.id` asserted-and-verified on raw bodies** (INV-3 — an adopted native-exporter property C25 relies on and the fixture checks, not one C25 implements) — the contract C24 needs to build the CXDB parent-chain (G26) and C27 needs for session mapping.
...
1. **G04 two-sinks misuse — highest.** Risk: an integrator wires OTLP → CXDB — the rejected path (CXDB has no OTLP receiver, AI-CONTEXT:210; OTLP→CXDB rejected, AI-CONTEXT §11.1:466 + §11.3:497; anti-edge enforced at C26 INV-2). De-risk first by publishing the two-sinks boundary (T5) as a shared, cross-referenced invariant and proving it in the fixture (AC-6) — that no CXDB post originates from the OTLP channel.
2. **`session.id` correlation property (feeds G26).** Risk: raw bodies land without the identity C24 needs for CXDB lineage. The attributes are Anthropic-native (C25 asserts, does not implement them); de-risk by **verifying** `session.id` on the body files early (T4/AC-5) before C24 builds its mapping rule — this is the seam C24's hard work (G26) depends on.
