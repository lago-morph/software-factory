...
- 6.6 **Revisit the `architecture-failure-mode-gate` CI skill.** During Phases 0-5 the gate was made inert (lint treats absent `architectures/failure-modes.md` as a no-op transition state, per the lint change made 2026-05-24). Phase 6 produces v3 architecture spec files at `architectures/v3/*.md` and a v3-shaped failure-mode catalog at [`failure-modes-v3`](architectures/v3/failure-modes-v3.md) with greenfield/brownfield severity columns rather than per-architecture columns. The gate must now be decided: either (a) **redesign** to enforce 1:1 correspondence between v3 architecture files and the v3 catalog's severity-and-mitigation cells (note the schema differs from the v1/v2 per-architecture matrix the gate was originally built for), or (b) **retire** the gate alongside the archived v1/v2 material and rely on Phase-3 adversarial passes + Phase-7 back-fill audit to enforce coverage discipline going forward. Decision-owner: lead agent on Phase-6 completion.
