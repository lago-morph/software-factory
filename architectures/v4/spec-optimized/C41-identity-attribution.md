...
**Outbound:**
- → **C23** every `attribute`/`sign` participates in the append-only event; C23 supplies the ordered gap-free `event_id` that C41 keys its provenance chain on (D-5 — C41 owns the chain, C23 owns the ordered ids).
