...
The new **report 29 — Prompt Engineering Survey** ([`29-prompt-engineering-survey`](../29-prompt-engineering-survey.md), drained 2026-05-16 from Schulhoff et al.'s *The Prompt Report*, arXiv:2406.06608v6) is the **academic taxonomy anchor for this thread's prompt-injection / jailbreaking discussion**. Report 29 §4 (*§5 Prompting Issues — the failure-mode taxonomy*) summarizes Schulhoff et al.'s five-issue catalogue:
...
**Implication for the factory.** Every architecture in [`00-comparison`](../../architectures/00-comparison.md) puts the implementer agent *inside* the trifecta by default: private data (repo + secrets), untrusted content (issues, PR comments, scraped docs, web search), external comms (git push, npm install, web fetch). F12 is not a hypothetical — it is the default state of an unconstrained implementer. The four architectures' F12 entries are all leg-3 mitigations (break the exfiltration vector). None of them break leg 1 or leg 2.

**Practitioner-side restatement — Shapiro's "do not give it production scissors" (Cluster K, 2026-05-13).** Dan Shapiro's post *"Completion, Chat, Agent, Claw"* (drained as [`32-shapiro-completion-chat-agent-claw`](../32-shapiro-completion-chat-agent-claw.md)) contains the corpus' first **named-practitioner real-world incident-report** of the Lethal Trifecta — his first OpenClaw install shredded itself by accumulating venture-capitalist emails, banker Chrome cookies, and crypto-wallet keys within hours of deployment. Shapiro names the failure pattern in his own voice and credits Willison: *"You could not ask for a better example of what Simon Willison calls 'The Lethal Trifecta'."* His rebuild rules — **read-anything-but-only-draft**, **thumbprint every artifact**, and *"do not give it production scissors"* (see report 32 §5.2) — are an empirical-practitioner restatement of the same closure rules CaMeL formalises in code (§3 below): trifecta closure at the integration/API boundary rather than inside a typed interpreter. R3 (*"do not give it production scissors"*) is corpus-shorthand-worthy as the operator-language complement to Willison's three legs and CaMeL's PI-SEC game. The Shapiro post is also primary anchor for **F44 — Lethal-Trifecta Production-Scissors Default** (report 32 §8.2), which names the *default* shape of an unconstrained personal/workplace Claw and motivates the need to bake R1–R5 into substrate (a "claw printer" issuing per-employee Claws with the closure rules pre-applied) rather than relying on per-Claw operator discipline at scale.
...
[`00-comparison`](../../architectures/00-comparison.md) §2.4 currently summarises F12 in a single cell per architecture. Here is what each architecture actually buys, in light of the primitives above.
...
### 6.4-bis Cross-reference — operational-deployment companion (report 18 §4.3 / §4.4)

[`18-openai-codex-substrate`](../18-openai-codex-substrate.md) §4.3 ("The `.rules` DSL — Starlark-anchored auditable auto-rejection") and §4.4 ("Operational posture at OpenAI — the agent-native telemetry stack") were drained on 2026-05-16 from manual MHTML capture of `developers.openai.com/codex/rules` and `openai.com/index/running-codex-safely/`. They are the operational-deployment companion to this thread's threat-model framing: §4.3 documents Codex's Starlark `prefix_rule(...)` DSL with inline `match`/`not_match` unit tests and `codex execpolicy check` CI harness (a sandboxed-DSL choice that mirrors CaMeL's side-effect-free interpreter substrate, §3 above — the policy itself can't exfiltrate during evaluation); §4.4 documents OpenAI's own internal posture, including (i) `Auto-review` mode delegating routine approvals to an auto-approval subagent (operational extension of §3's Dual-LLM / planner-summariser split — Codex's reviewer agent is the *capability gate* between the planner and the sandbox boundary), (ii) a five-key managed network policy (`allowed_web_search_modes`, `[experimental_network]` `enabled` / `allow_local_binding` / `denied_domains` / `allowed_domains`) that pins the trifecta closure at admin scope, and (iii) **OpenTelemetry log export** of five event categories (user prompts, tool approval decisions, tool execution results, MCP server usage, network-proxy allow/deny) feeding an internal **AI-powered security triage agent** that reconstructs *intent* (the "why") rather than SIEM-style "what" — the operational answer to T1–T3 forensic-attribution gaps named in §5 of this thread. Factory implication: every architecture in §6 should adopt the OTEL five-event taxonomy and the triage-agent layer as standing telemetry — independent of which sandbox primitive each architecture chooses.
...
- **(Cluster-O, 2026-05-16) Does the Lethal Trifecta have a multi-agent generalisation?** Neves & Bussmann ([`37-academic-llm-agent-collusion`](../37-academic-llm-agent-collusion.md)) document that two LLM-driven agents in a shared market context converge on **tacit collusion** — supra-competitive equilibria without explicit coordination signals — and that inter-agent communication produces a *"mimicking concerns about collusion"* sub-effect in which agents discuss the failure mode then implement a softer / deniable version of it. CaMeL closes the Trifecta via a typed-interpreter boundary in a *single-agent* setting (paper §3 throughout); the **open question** is whether the same closure-rule applies when two CaMeL-protected agents share a market environment, an observable action history, or a shared training distribution. **Candidate F48 (Tacit-Collusion-via-Shared-Context) and F49 (Discussion-as-Amplification)** proposed in report 37 §8.1 sit on the multi-agent generalisation surface. Flagged for a future drain: examine whether `defns.tex`'s formal capability model has a multi-agent extension (each agent's `capability_t` is augmented with a *shared-environment* provenance tag), and whether the AgentDojo benchmark has any multi-agent attack scenarios. The pragmatic implication for the factory: the substrate-level guarantee CaMeL provides at the single-agent boundary may not transfer to multi-agent fleets (Schillace compounding teams, report 28 §3.4; Cherny 5-agents-steady-state, followup/03) without additional cross-agent capability composition rules.
