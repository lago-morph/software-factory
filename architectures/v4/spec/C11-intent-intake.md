...
>
> **The fill is *graded*, not uniformly weak.** Two slots have firmer anchors than the others: field #7 (DoD) is grounded in a first-class inventory atom **A94** ("Definition-of-Done — acceptance-criteria checkbox set driving build loops"), not merely the Kilroy `DoD.md` file; and fields #6 (Constraints) and #8 (Known ambiguities) track the named **requirement-classes** and **ambiguity** Part-2 attributes almost 1:1. The slots most at risk of diverging from the real GF-C set are the **boundary/partition** ones — the Scope (#2) vs Non-goals (#3) vs Inputs (#5) split that carves the single "completeness/specificity" attribute space into three. Those are the cells OQ-1 most needs to confirm.
...
| Identity | One record per unit of intended work; it anchors the C08 spec a human then authors. Per-record provenance is `created_by` (C41); GF-C `transfused_from` is **pack-level**, not on the record (INV-4, A93). |
| Lifecycle | Authored (9 slots filled) → committed (versioned + attributable via `created_by`/C41, like C08) → consumed by C08 authoring → (optionally) its DoD field seeds a C53 rubric. On a fix/iteration, the record is *revised* alongside the spec. |
...
- **Attribution / governance.** Each record is a git commit carrying actor identity via C41 (`created_by`). The crucible pack's `transfused_from` (GF-C) lineage is carried at the **component** level per C51/A93 — not on each record (INV-4).
...
3. **AC-3 (versioned + attributable).** Each intent record is a git commit carrying actor identity (`created_by`, C41); the crucible **pack** carries the GF-C `transfused_from` lineage at component grain (C51/A93), not the per-record artifact (INV-4).
