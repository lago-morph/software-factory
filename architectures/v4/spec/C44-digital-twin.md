...
| Verifier (sibling, same batch) | **C45** Twin contract & fidelity verification | C45 verifies this twin's behavior matches the real service and its usage matches service promises (G22). C44 **provides** the twin + the declared cloned-surface contract C45 checks against. C45 depends on C44 (inventory C45). *On disk (sweep-1); C45 §1/§3 confirm it owns the fidelity predicate, not the twin. C45's recorded reference may consume C44's I3 capture (C45:OQ-C45-2).* |
| Isolator (consumer, same batch) | **C43** Isolation & lethal-trifecta boundary | C43 uses this twin to bound blast radius (typed Bash/net/fs; production-scissors-by-declaration), G31, per D-13. C43 depends on C44 (inventory C43). C44 **provides** the standalone twin C43 isolates behind. *On disk (sweep-1); C43 §1/§3 confirm it owns the blast-radius bound + the deterministic twin-by-default routing (F44) — that routing is C43's, not C44's.* |
| Consumers (run target) | **C30/C31** Scenario store + runner | Scenarios run "thousands per hour without rate limits" *against the twin* (README:195); in an eval context the scenario/runner selects the twin as run target (README:499; C31:OQ-5). C44 is the addressable target, not the selector — and the deterministic *twin-by-default routing* for the broad-tool agent is **C43's** (F44), a distinct axis from eval run-target selection. |
...
| I3 | **Record/replay fixture I/O** | internal/data | Serve previously-recorded real-service traffic for matching requests (VCR.py/go-vcr/HoverFly "capture and replay"); capture-mode populates fixtures from real traffic. *(These captured fixtures are also a candidate source for **C45's** recorded fidelity *reference* — the reference home is C45:OQ-C45-2; C44 owns the capture, not the fidelity verdict.)* | C44 (this); C45 may consume the capture as reference |
