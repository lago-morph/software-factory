...
**Cross-reference — Caremark-line board-exposure framing (2026-05-16 Cluster-J drain).** Report 31 ([`31-caremark-rsi-board-exposure`](31-caremark-rsi-board-exposure.md)) reads the StrongDM "code must not be reviewed by humans" architectural posture through Delaware *Caremark*-line duty-of-oversight doctrine: this is — per Kahana 2026-03-17 (*The Ungovernable Machine*) — the *paradigm case* of a design choice that disables board-level monitoring and that the Boeing / Hughes-line case law treats as potentially actionable. Whether the Attractor architecture meets Kahana's three-part RSI test (durable self-modification + compounding ability + limited human gating) depends on per-deployment configuration of skill-write persistence and per-modification human-review gating; the architectural defaults documented above are operationally accurate but governance-incomplete read against the Caremark frame. See also followup/10 §1.1 (Kahana on StrongDM's "tracing difficult by design" inversion of software-liability allocation) and followup/10 §6c (Cluster-J dedicated reports).
